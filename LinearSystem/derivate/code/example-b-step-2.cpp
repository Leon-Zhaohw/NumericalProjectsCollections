...
T compute(const V x[3],V c,T a,T b,T InputArea,V dE[3],M ddE[3][3])
{
    ...
    V u=x[0]-x[2];
    V v=x[1]-x[2];
    T d_min = z.dot(-u.cross(v-u))/((-u.cross(v-u)).mag());
    T cot_alpha = (-u.dot(v-u))/((-u.cross(v-u)).mag());
    T cot_beta = ((u-v).dot(-v))/(((u-v).cross(-v)).mag());
    T cot_gamma = ((v).dot(u))/(((v).cross(u)).mag());
    T E_Dirichlet = 1/(d_min*d_min)*(cot_alpha*(v).mag2() + cot_beta*(u).mag2() + cot_gamma*(u-v).mag2());
    T E_area = 1/(d_min*d_min)*(((u).cross(v)).mag2())/InputArea;
    T E = a*E_Dirichlet + b*E_area;
    return E;
}
