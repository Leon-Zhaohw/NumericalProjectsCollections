...
// ddE = Hessian : dF;
T compute(const V& sig, const M& UU, const M& dF, M& dE, M& ddE)
{
    diff_helper<T,3> h;
    V diag_dE;
    ...
    h.B = h.A;
    h.flip=false;
    dE = UU * diag(diag_dE) * UU.t();
    ddE = UU * h.apply(UU.t() * dF * UU) * UU.t();
    return E;
}
...
int main(int argc, char* argv[])
{
    ...
    T E0=compute(Sig,UU,dF,dE0,ddE0);
    T E1=compute(Sig1,UU1,dF,dE1,ddE1);
    T d0=(E1-E0)/eps;
