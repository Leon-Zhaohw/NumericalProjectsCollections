...
struct HELPER
{
    ...
    void fill()
    {
        ...
        dW.set("rs",A_X*(lambda*dB("rs")*B + 2*mu*dE_bar("ijrs")*E_bar("ji")));

        symbolic_tensor A("ijsv",0.5*(R_X_inv("si")*R_X_inv("vj")+R_X_inv("vi")*R_X_inv("sj")));
        symbolic_tensor Ac("sv",R_X_inv("si")*R_X_inv("vi"));
        symbolic_tensor C("sv",lambda*B*Ac("sv") + mu*(R_X_inv("si")*R_X_inv("vj")+R_X_inv("vi")*R_X_inv("sj"))*E_bar("ji"));
        ddW.set("rsuv",A_X*(id3("ru")*C("sv")+lambda*dB("rs")*dE_bar("jjuv") + 2*mu*dE_bar("ijrs")*dE_bar("jiuv")));
    }
};
