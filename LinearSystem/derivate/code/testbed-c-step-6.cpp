...
struct HELPER
{
    ...
    void fill()
    {
        ...
        e.set("",h*dZ("i")*u("i"));
        de.set("r",dh("r")*dZ("i")*u("i")+h*ddZ("ij")*u("i")*dq("jr")+h*dZ("i")*du("ir"));

        f.set("",h*h/2*ddZ("ij")*u("i")*u("j"));
        df.set("r",h*dh("r")*ddZ("ij")*u("i")*u("j")+h*h/2*dddZ("ijk")*u("i")*u("j")*dq("kr")+h*h/2*ddZ("ij")*du("ir")*u("j")+h*h/2*ddZ("ij")*u("i")*du("jr"));

        g.set("",Z);
        dg.set("r",dZ("i")*dq("ir"));

        psi.set("",Z+e+f);
        dpsi.set("r",dg("r")+de("r")+df("r"));

// \psi &= Z + h dZ\cdot\uu + \frac{1}{2} h^2 \uu^TddZ\uu
    }
};
