...
struct HELPER
{
    ...
    void fill()
    {
        ...
        dE_dN.set("r",-2*g*g*d*C*z("r")+g*h*N("r"));
        dE_dz.set("r",-2*g*g*d*C*N("r"));

        symbolic_tensor dN_dx[3];
        dN_dx[0].set("ir",e("irk")*v("k"));
        dN_dx[1].set("ir",e("ijr")*u("j"));
        dN_dx[2].set("ir",-e("irk")*v("k")-e("ijr")*u("j"));
        int dz_dx[3]={1,0,0};

        for(int i=0;i<3;i++) dE[i].set("r",dE_dN("i")*dN_dx[i]("ir")+dz_dx[i]*dE_dz("r"));

        symbolic_tensor m=2*g*g*(4*g*d*d-1);
        symbolic_tensor p("r",m*C*z("r")-2*g*g*d*h*N("r"));
        ddE_dN_dN.set("rs",p("r")*z("s")+g*((6*a/A+8*f)*N("r")-2*g*d*h*z("r"))*N("s")+g*h*id("rs"));
        ddE_dN_dz.set("rs",p("r")*N("s")-2*g*g*d*C*id("rs"));
        ...
        for(int i=0;i<3;i++)
        {
            for(int j=0;j<3;j++)
            {
                ddE[i][j].set("rs",
                    ...
                    ddE_dN_dz("is")*dN_dx[i]("ir")*dz_dx[j]+
                    ddE_dN_dz("jr")*dz_dx[i]*dN_dx[j]("js")+
                    ddE_dz_dz("rs")*dz_dx[i]*dz_dx[j]+
