...
struct HELPER
{
    ...
    symbolic_tensor A2,ddA2_dN_dN;
    symbolic_tensor A,ddA_dN_dN;
    symbolic_tensor d,dd_dN,dd_dz,ddd_dN_dN,ddd_dN_dz,ddd_dz_dz;
    symbolic_tensor d2,ddd2_dN_dN,ddd2_dN_dz,ddd2_dz_dz;
    symbolic_tensor g,ddg_dN_dN,ddg_dN_dz,ddg_dz_dz;
    symbolic_tensor C,ddC_dN_dN;
    ...
    void fill()
    {
        ...
        ddA_dN_dN.set("rs",(id("rs")-N("r")*N("s")/A2)/A);
        ddd_dN_dN.set_zero({3,3});
        ddd_dN_dz.set("rs",id("rs"));
        ddd_dz_dz.set_zero({3,3});
        ddd2_dN_dN.set("rs",2*z("s")*z("r"));
        ddd2_dN_dz.set("rs",2*d*ddd_dN_dz("rs")+2*N("s")*z("r"));
        ddd2_dz_dz.set("rs",2*N("s")*N("r"));
        ddg_dN_dN.set("rs",2*g*g*g*2*d*z("s")*2*d*z("r")-g*g*ddd2_dN_dN("rs"));
        ddg_dN_dz.set("rs",2*g*g*g*2*d*N("s")*2*d*z("r")-g*g*ddd2_dN_dz("rs"));
        ddg_dz_dz.set("rs",2*g*g*g*2*d*N("s")*2*d*N("r")-g*g*ddd2_dz_dz("rs"));
        ddC_dN_dN.set("rs",(3*a*A+2*f*A2)*ddA2_dN_dN("rs")+(3*a/A*N("s")+2*f*2*N("s"))*2*N("r"));
        ddE_dN_dN.set("rs",ddg_dN_dN("rs")*C+g*ddC_dN_dN("rs")-g*g*2*d*z("r")*h*N("s")-g*g*2*d*z("s")*h*N("r"));
        ddE_dN_dz.set("rs",ddg_dN_dz("rs")*C-g*g*2*d*N("s")*h*N("r"));
