...
struct HELPER
{
    ...
    void fill()
    {
        ...
        ddd2_dN_dz.set("rs",2*d*id("rs")+2*N("s")*z("r"));
        ddg_dN_dN.set("rs",2*g*g*(4*g*d*d-1)*z("s")*z("r"));
        ddg_dN_dz.set("rs",2*g*g*g*2*d*N("s")*2*d*z("r")-g*g*ddd2_dN_dz("rs"));
        ddg_dz_dz.set("rs",2*g*g*(4*g*d*d-1)*N("s")*N("r"));
        ddC_dN_dN.set("rs",h*id("rs")+(6*a/A+8*f)*N("s")*N("r"));
