...
T psi(V x,V v,V& dE,M& ddE,M& v_dddE,M& vv_ddddE,V& RdE,V& RvddE,V& RvvdddE)
{
    V o=ones<T,3>();
    T a=x.dot(x);
    T b=x.dot(v);
    ...
    RdE=2*c*o;
    RvddE=-s*4*b*o+2*c*diff_over_diff(v,x);
    RvvdddE=diff_over_diff(-8*c*b*x*b-4*s*v*b-4*s*x*d-4*s*b*v,x);
...
T compute(V x,V r,T a,V& dE,M& ddE,V& RdE)
{
    ...
    T Z=psi(q,v,dZ,ddZ,v_dddZ,vv_ddddZ,RdZ,RvddZ,RvvdddZ);
    V vddZ=ddZ*v;
    V vvdddZ=v_dddZ*v;
    ...
    RdE=RdZ*s+RvddZ*s*t+2*s*s*q*(vvvdddZ*t2h/Ts)+RvvdddZ*s*s*t2h;
