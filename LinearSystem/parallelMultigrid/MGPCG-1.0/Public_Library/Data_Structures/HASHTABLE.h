//#####################################################################
// Copyright 2002-2007, Robert Bridson, Kevin Der, Ronald Fedkiw, Geoffrey Irving, Neil Molino, Craig Schroeder, Andrew Selle, Tamar Shinar, Joseph Teran.
...
#include <Arrays/PROJECTED_ARRAY.h>
#include <Math_Tools/Hash.h>
#include <Math_Tools/integer_log.h>
namespace PhysBAM{
...
public:
    ...
    ARRAY<ENTRY> old_table;ARRAY<ENTRY>::Exchange_Arrays(old_table,table);Initialize_New_Table(estimated_max_number_of_entries);
...
private:
    ...
    template<class T2> typename boost::disable_if<IS_SAME<T2,void> >::type
...
public:
    ...
    {ARRAY<ENTRY>::Exchange_Arrays(hash1.table,hash2.table);exchange(hash1.number_of_entries,hash2.number_of_entries);exchange(hash1.next_resize,hash2.next_resize);}
    ...
    void Print_All_Keys(std::ostream& output)
    {for(int h=1;h<=table.m;h++) if(table(h).state==ENTRY_ACTIVE) output<<table(h).data<<std::endl;}
    ...
    void Get_Keys(LIST_ARRAY<TK>& keys) const
    {keys.Remove_All();keys.Preallocate(Size());for(int h=1;h<=table.m;h++) if(table(h).state==ENTRY_ACTIVE) keys.Append(table(h).key);}

    void Get_Data(LIST_ARRAY<T_UNLESS_VOID>& data) const
    {data.Remove_All();data.Preallocate(Size());for(int h=1;h<=table.m;h++) if(table(h).state==ENTRY_ACTIVE) data.Append(table(h).data);}

    template<class RW>
    void Read(typename IF<IS_SAME<T,void>::value,std::istream&,UNUSABLE>::TYPE input) // void version
    {int entries;Read_Binary<RW>(input,entries);Initialize_New_Table(entries);
    for(int i=1;i<=entries;i++){TK key;Read_Binary<RW>(input,key);Insert(key);}}

    template<class RW>
    void Read(typename IF<IS_SAME<T,void>::value,UNUSABLE,std::istream&>::TYPE input) // non-void version
    {int entries;Read_Binary<RW>(input,entries);Initialize_New_Table(entries);
    for(int i=1;i<=entries;i++){TK key;T value;Read_Binary<RW>(input,key,value);Insert(key,value);}}

    template<class RW>
    void Write(typename IF<IS_SAME<T,void>::value,std::ostream&,UNUSABLE>::TYPE output) const // void version
    {Write_Binary<RW>(output,number_of_entries);
    for(int h=1;h<=table.m;h++) if(table(h).state==ENTRY_ACTIVE) Write_Binary<RW>(output,table(h).key);}

    template<class RW>
    void Write(typename IF<IS_SAME<T,void>::value,UNUSABLE,std::ostream&>::TYPE output) const // non-void version
    {Write_Binary<RW>(output,number_of_entries);
    for(int h=1;h<=table.m;h++) if(table(h).state==ENTRY_ACTIVE) Write_Binary<RW>(output,table(h).key,table(h).data);}
