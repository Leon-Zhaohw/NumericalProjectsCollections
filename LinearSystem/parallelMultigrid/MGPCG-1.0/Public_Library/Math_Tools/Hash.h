...
template<class TV> inline HASH Hash_Reduce(const BOX<TV>& key){return HASH(key.min_corner,key.max_corner);}
...
{return HASH(RAW_ARRAY<const char>((int)strlen(key),key));}

inline HASH Hash_Reduce(const std::string& key)
{return HASH(RAW_ARRAY<const char>((int)key.length(),key.c_str()));}
...
template<class T,class T_ARRAYS> inline HASH Hash_Reduce(const ARRAYS_ND_BASE<T,T_ARRAYS>& key)
{return HASH(key.Derived().Domain_Indices(),key.array);}

template<class T,int length> inline HASH Hash_Reduce(const FACE_ARRAYS_1D<T,length>& key){return Hash_Reduce(key.u);}
template<class T,int length> inline HASH Hash_Reduce(const FACE_ARRAYS_2D<T,length>& key){return HASH(key.u,key.v);}
template<class T,int length> inline HASH Hash_Reduce(const FACE_ARRAYS_3D<T,length>& key){return HASH(key.u,key.v,key.w);}
