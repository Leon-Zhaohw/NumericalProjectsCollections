...
public:
    ...
    explicit VECTOR(const LIST_ARRAY<T>& v)
        :x(v(1)),y(v(2)),z(v(3))
    {
        assert(3==v.Size());
    }
    ...
    template<class T_VECTOR> typename boost::enable_if<boost::mpl::and_<IS_SAME<T,typename T_VECTOR::ELEMENT>,INTS_EQUAL<T_VECTOR::m,3> >,VECTOR&>::type
    ...
    {STATIC_ASSERT((mpl::and_<mpl::less_equal<mpl::int_<1>,mpl::int_<d1> >,mpl::less_equal<mpl::int_<d2>,mpl::int_<3> > >::value));
    ...
    MATRIX<T,3> Cross_Product_Matrix() const
    {return MATRIX<T,3>(0,z,-y,-z,0,x,y,-x,0);}
...
operator>>(std::istream& input,VECTOR<T,3>& v)
{return input>>v.x>>v.y>>v.z;}

template<class T> inline std::ostream&
operator<<(std::ostream& output,const VECTOR<T,3>& v)
