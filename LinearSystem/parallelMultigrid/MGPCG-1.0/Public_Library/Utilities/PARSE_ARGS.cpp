...
Print_Usage(bool do_exit) const
{
    ...
    for(i=1;i<=arg_data_list.m;i++){arg_data_list(i).Print_Synopsis(LOG::cerr);LOG::cerr<<" ";}
    ...
    for(i=1;i<=arg_data_list.m;i++){arg_data_list(i).Print_Description(LOG::cerr,width+2);LOG::cerr<<std::endl;}
...
Find_And_Remove(const char *str,int& argc,char** argv)
{
    ...
    if(i<argc){for(;i<argc-1;i++)argv[i]=argv[i+1];argc--;return true;}
    return false;
}
//#####################################################################
...
{
    LOG::cout<<"command = ";
    for(int i=0;i<argc;i++) LOG::cout<<argv[i]<<' ';
    LOG::cout<<std::endl;
    LOG::cout<<"working directory = "<<FILE_UTILITIES::Get_Working_Directory()<<std::endl;
}
//#####################################################################
