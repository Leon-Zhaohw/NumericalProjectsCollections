...
END SUBROUTINE Tri_RL
  ...
  SELECT CASE (CDIM)
     ...
     CASE DEFAULT
        U=0.0
        IF(numpe==1)WRITE (12,*)'Cdim not equal 2 or 3 in Function U(...)'
  END SELECT
  END FUNCTION U
...
READ(11,'(A80)') Title
IF(numpe==1)WRITE(12,*)'Project:',Title
READ(11,*) Cdim
IF(numpe==1)WRITE(12,*)'Cartesian_dimension:',Cdim
READ(11,*) N_dof        !    Degrees of freedom per node
IF(N_Dof == 1) THEN
        IF(numpe==1)WRITE(12,*)'Potential Problem'
ELSE
        IF(numpe==1)WRITE(12,*)'Elasticity Problem'
END IF
IF(N_dof == 2)THEN 
        READ(11,*) Toa   !Toa ....Type of analysis
                        ! (solid plane strain = 1,solid plane stress = 2)
	IF(Toa == 1)THEN
                IF(numpe==1)WRITE(12,*)'Type of Analysis: Solid Plane Strain'
	ELSE
                IF(numpe==1)WRITE(12,*)'Type of Analysis: Solid Plane Stress'
	END IF
END IF	
READ(11,*) Nreg       !   Type of region
IF(NReg == 1) THEN
        IF(numpe==1)WRITE(12,*)'Finite Region'
ELSE
        IF(numpe==1)WRITE(12,*)'Infinite Region'
END IF
READ(11,*) Isym       !   Symmetry code
SELECT CASE (isym)
CASE(0)
IF(numpe==1)WRITE(12,*)'No symmetry'
CASE(1) 
IF(numpe==1)WRITE(12,*)'Symmetry about y-z plane' 
CASE(2)
IF(numpe==1)WRITE(12,*)'Symmetry about y-z and x-z planes'
CASE(3) 
IF(numpe==1)WRITE(12,*)'Symmetry about all planes'
END SELECT
READ(11,*) Ltyp        !   Element type
IF(Ltyp == 1) THEN
IF(numpe==1)WRITE(12,*)'Linear Elements'
ELSE
IF(numpe==1)WRITE(12,*)'Quadratic Elements'
END IF
...
IF(N_dof == 1) THEN
        READ(11,*) Con
        IF(numpe==1)WRITE(12,*)'Conductivity=',Con
ELSE	
        READ(11,*) E,ny
	IF(ToA == 2)THEN		!	Solid Plane Stress
		...
		ny =ny/(1+ny)
	END IF			
        IF(numpe==1)WRITE(12,*)'Modulus:',E
        IF(numpe==1)WRITE(12,*)'Poissons ratio:',ny
END IF

READ(11,*) Nodes
IF(numpe==1)WRITE(12,*)'Number of Nodes of System:',Nodes
READ(11,*) Maxe     
IF(numpe==1)WRITE(12,*)'Number of Elements of System:', Maxe
RETURN
...
DO nr=1,Nregs
	...
	!   Read properties
        IF(N_dof == 1) THEN
                READ(11,*) ConR(nr)
                IF(numpe==1)WRITE(12,*)'Conductivity=',ConR(nr)
	ELSE	
		...
		END IF			
                IF(numpe==1)WRITE(12,*)'Youngsmodulus:',ER(nr)
                IF(numpe==1)WRITE(12,*)'Poissons ratio:',nyR(nr)
	END IF
        READ(11,*)Nbel(nr)
!	IF(Nbel(nr) > MaxeR)MaxeR= Nbel(nr)
        READ(11,*)(ListR(nr,nb),nb=1,Nbel(nr))
        IF(numpe==1)WRITE(12,*) ' List of Boundary Elements: '
        IF(numpe==1)WRITE(12,*)(ListR(nr,nb),nb=1,Nbel(nr))
END DO &
Region_loop
RETURN
...
IF(numpe==1)WRITE(12,*)''
IF(numpe==1)WRITE(12,*)'Elements with Dirichlet BC´s: '
IF(numpe==1)WRITE(12,*)''
...
IF(NE_u > 0) THEN
DO n=1,NE_u
        READ(41,*) Nel,(Elres_u(Nel,m),m=1,Ndofe)                 
!       READ(41,*) Nel,(BCode(Nel,m),m=1,Ndofe)
END DO  
	BCode(Nel,:)=1
        IF(numpe==1)WRITE(12,*)'Element ',Nel,'  Prescribed values: '
	Na= 1
        DO M= 1,Nodel , Nodel - 1
                IF(numpe==1)WRITE(12,*) Elres_u(Nel,na:na+n_dof-1)
                Na= na+N_dof
        END DO 
END IF
IF(numpe==1)WRITE(12,*)''
IF(numpe==1)WRITE(12,*)'Elements with Neuman BC´s: '
IF(numpe==1)WRITE(12,*)''
Elres_t(:,:)=0   !   Default prescribed values = 0.0
READ(41,*)NE_t            
DO n=1,NE_t
        READ(41,*) Nel,(Elres_t(Nel,m),m=1,Ndofe)
END DO  
        IF(numpe==1)WRITE(12,*)'Element ',Nel,'  Prescribed values: '
	Na= 1
        DO M= 1,Nodel, Nodel - 1
                IF(numpe==1)WRITE(12,*) Elres_t(Nel,na:na+n_dof-1)
                Na= na+N_dof
        END DO 
RETURN
...
!-------------------------------------------------------
DO Node=1,Nodes
 READ(21,*) (xP(M,Node),M=1,Cdim)
END DO
DO Node=1,Nodes,Nodes-1
 IF(numpe==1)WRITE(12,'(A5,I5,A8,3F8.2)') 'Node ',Node,&
...
END IF
	DO Nel=1,Maxe
           READ(31,*) (Inci(Nel,n),n=1,Nodel)
        END DO
DO Nel = 1 , Maxe, Maxe - 1
IF(numpe==1)WRITE(12,'(A3,I5,A8,24I5)')'EL ',Nel,'  Inci  ',Inci(Nel,:)
END DO 
RETURN
