...
{ 
  ...
  ShadowCoord = gl_TextureMatrix[4] * vec4(Pos.xyz, 1.0);
  vec2 shadowOutside = ShadowCoord.xy / ShadowCoord.w;
  if (abs(shadowOutside.x) <= 1.0 && abs(shadowOutside.y) <= 1.0)
 ...
 // Mask = min(Shadow, NdotL);
  ...
  //gl_FragColor = vec4((NdotL + 0.01 * Mask), 1.0);
  gl_FragColor = vec4(Mask, 1.0);
  //ShadowCoord.xyz = ShadowCoord.xyz/ShadowCoord.w;
  //gl_FragColor = vec4(vec3(ShadowCoord.xyz), 1.0);

  //vec3 shadowvalue = texture2DProj(shadowTex, ShadowCoord).rgb;
  //gl_FragColor = vec4(shadowvalue, 1.0);
