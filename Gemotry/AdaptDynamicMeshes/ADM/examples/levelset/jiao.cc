...
class Jiao : public CPLS3D::Velocity3D
{
protected:
  double s[3];
  
public:
  Jiao(int nx, int ny, int nz) 
  { 
    s[0] = double(nx);
    s[1] = double(ny);
    s[2] = double(nz);
  ...
  void GetVelocity(const CPLS3D::Vec3D& p, CPLS3D::Vec3D& u)
  {
    ...
    u = CPLS3D::Vec3D( s[0]*std::pow(std::sin(M_PI*x),2)*( std::sin(2*M_PI*z) - std::sin(2*M_PI*y) ),
		       s[1]*std::pow(std::sin(M_PI*y),2)*( std::sin(2*M_PI*x) - std::sin(2*M_PI*z) ),
		       s[2]*std::pow(std::sin(M_PI*z),2)*( std::sin(2*M_PI*y) - std::sin(2*M_PI*x) ) );
...
int main(int argc, char** argv)
{
  ...
  double dt = 4.8 / (2*double(nx) + 2*double(ny) + 2*double(nz));
  ...
  CPLS3D::PLS3D pls( nx, ny, nz, HH, 
		     Sphere( nx, ny, nz, CPLS3D::Vec3D(0.5*nx,0.75*ny,0.5*nz), radius ),
		     &(vel), dt );
  ...
  ADM::AdaptiveMesh mesh( "./jiao100b.off", &criteria );
  mesh.set_threshold( radius*0.01 );
  mesh.set_stiffness( -1 );
  mesh.set_smooth_steps( 5 );

  ADMViewer win( "Jiao", &mesh, &surf, &criteria );
  //win.KeyboardOptions();
  win.show();
  Fl::run();
  ...
  /*
    ...
    mesh.deform();
    mesh.adapt();
    sprintf( file, "m_jiao%.3f.off", vel.time() );
    mesh.save_mesh( file );
    //sprintf( file, "pov/jiao/jiao%.3f.inc", vel.time() );
    //save_pov( mesh, file );
