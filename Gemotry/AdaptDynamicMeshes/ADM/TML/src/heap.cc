...
void MxHeap::upheap(unsigned int i)
{
  ...
  while( index>0 && cmp(moving->heap_key(),ref(p)->heap_key()) )
    {
      place(ref(p), index);
      index = p;
      p = parent(p);
...
void MxHeap::downheap(unsigned int i)
{
  ...
  while( l<length() )
    {
      if( r<length() && cmp(ref(r)->heap_key(),ref(l)->heap_key()) )
	largest = r;
      else 
	largest = l;
      
      if( cmp(ref(largest)->heap_key(),moving->heap_key()) )
	{
...
void MxHeap::update(MxHeapable *t, double v)
{
  ...
  if( i>0 && cmp(v,ref(parent(i))->heap_key()) )
    upheap(i);
  else
    downheap(i);
}
...
MxHeapable *MxHeap::remove(MxHeapable *t)
{
  ...
  if( cmp(t->heap_key(),ref(i)->heap_key()) )
    downheap(i);
  else
    upheap(i);
