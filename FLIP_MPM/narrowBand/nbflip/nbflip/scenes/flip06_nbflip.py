...
print("Narrow band enabled: %i" % narrowBand)
print('Solver grid resolution is: %i x %i x %i' % (gs.x, gs.y, gs.z))
...
while s.frame < 200:
	...
	if narrowBand:
		...
		phi.join( phiParts );
		extrapolateLsSimple(phi=phi, distance=narrowBandWidth+2, inside=True, flags=flags, ignoreWalls=True)
	else:
