...
void ActVoxelGrid::loadFromOccGridRLE(OccGridRLE *ogSrc, unsigned char
																			maxD2Dist, bool conserveMemory = false)
{
	// load the dimensions of the grid
	xdim = ogSrc->xdim;
	ydim = ogSrc->ydim;
...
void ActVoxelGrid::blur(unsigned char d1limit, unsigned char d2limit)
{
	...
	for (iz=0; iz<zdim; iz++) {
		...
		swap(sweeps[0], sweeps[1]);
		swap(usedScanline[0],  usedScanline[1]);
	}
																	 ...
																	 unsigned char d2limit)
{
	...
	if (lastY != iy - 1) {
		...
		swap(tempRange, possibleD2);
		swap(tempRangeLen, possibleD2Len);
	}
