	...
	double				eps)			// the error bound
{
	...
	ANNGlobal global;
	global.ANNkdDim = dim;						// copy arguments to static equivs
	global.ANNkdQ = q;
	global.ANNkdPts = pts;
	
	ANNVisit visit;
	visit.ANNmaxPtsVisited = 0;	// maximum number of pts visited
	visit.ANNptsVisited = 0;
	
	if (k > n_pts) {					// too many near neighbors?
		annError("Requesting more near neighbors than data points", ANNabort);
	}

	global.ANNkdMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating op count

	global.ANNkdPointMK = new ANNmin_k(k);		// create set for closest k points
										// search starting at the root

	root->ann_search(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim),visit,global);

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		dd[i] = global.ANNkdPointMK->ith_smallest_key(i);
		nn_idx[i] = global.ANNkdPointMK->ith_smallest_info(i);
	}
	delete global.ANNkdPointMK;				// deallocate closest point set
}
...
void ANNkd_split::ann_search(ANNdist box_dist, ANNVisit& visit, ANNGlobal& global )
{
										// check dist calc term condition
	if (visit.ANNmaxPtsVisited != 0 && visit.ANNptsVisited > visit.ANNmaxPtsVisited) return;

										// distance to cutting plane
	ANNcoord cut_diff = global.ANNkdQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		child[ANN_LO]->ann_search(box_dist,visit,global);// visit closer child first

		ANNcoord box_diff = cd_bnds[ANN_LO] - global.ANNkdQ[cut_dim];
		...
		if (box_dist * global.ANNkdMaxErr < global.ANNkdPointMK->max_key())
			child[ANN_HI]->ann_search(box_dist,visit,global);

	}
	else {								// right of cutting plane
		child[ANN_HI]->ann_search(box_dist,visit,global);// visit closer child first

		ANNcoord box_diff = global.ANNkdQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		box_dist = (ANNdist) ANN_SUM(box_dist,
				ANN_DIFF(ANN_POW(box_diff), ANN_POW(cut_diff)));
										...
										// visit further child if close enough
		if (box_dist * global.ANNkdMaxErr < global.ANNkdPointMK->max_key())
			child[ANN_LO]->ann_search(box_dist,visit,global);
...
void ANNkd_leaf::ann_search(ANNdist box_dist, ANNVisit& visit, ANNGlobal& global)
{
	...
	min_dist = global.ANNkdPointMK->max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = global.ANNkdPts[bkt[i]];			// first coord of next data point
		qq = global.ANNkdQ;					// first coord of query point
		dist = 0;

		for(d = 0; d < global.ANNkdDim; d++) {
		...
		if (d >= global.ANNkdDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			global.ANNkdPointMK->insert(dist, bkt[i]);
			min_dist = global.ANNkdPointMK->max_key();
		}
	}
