	...
	double				eps)			// error bound (ignored)
{
	...
	ANNGlobal global;
	global.ANNprMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating ops

	global.ANNprDim = dim;						// copy arguments to static equivs
	global.ANNprQ = q;
	global.ANNprPts = pts;
	
	ANNVisit visit;
	visit.ANNptsVisited = 0;					// initialize count of points visited

	global.ANNprPointMK = new ANNmin_k(k);		// create set for closest k points

										// distance to root box
	ANNdist box_dist = annBoxDistance(q,
				bnd_box_lo, bnd_box_hi, dim);

	global.ANNprBoxPQ = new ANNpr_queue(n_pts);// create priority queue for boxes
	global.ANNprBoxPQ->insert(box_dist, root); // insert root in priority queue

	while (global.ANNprBoxPQ->non_empty() &&
		(!(visit.ANNmaxPtsVisited != 0 && visit.ANNptsVisited > visit.ANNmaxPtsVisited))) {
		ANNkd_ptr np;					// next box from prior queue

										// extract closest box from queue
		global.ANNprBoxPQ->extr_min(box_dist, (void *&) np);

		ANN_FLOP(2)						// increment floating ops
		if (box_dist*global.ANNprMaxErr >= global.ANNprPointMK->max_key())
			break;

		np->ann_pri_search(box_dist,visit,global);	// search this subtree.
	}

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		dd[i] = global.ANNprPointMK->ith_smallest_key(i);
		nn_idx[i] = global.ANNprPointMK->ith_smallest_info(i);
	}

	delete global.ANNprPointMK;					// deallocate closest point set
	delete global.ANNprBoxPQ;					// deallocate priority queue
}
...
void ANNkd_split::ann_pri_search(ANNdist box_dist, ANNVisit& visit, ANNGlobal& global)
{
	ANNdist new_dist;					// distance to child visited later
										// distance to cutting plane
	ANNcoord cut_diff = global.ANNprQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		ANNcoord box_diff = cd_bnds[ANN_LO] - global.ANNprQ[cut_dim];
		...
		if (child[ANN_HI] != KD_TRIVIAL)// enqueue if not trivial
			global.ANNprBoxPQ->insert(new_dist, child[ANN_HI]);
										// continue with closer child
		child[ANN_LO]->ann_pri_search(box_dist,visit,global);
	}
	else {								// right of cutting plane
		ANNcoord box_diff = global.ANNprQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		new_dist = (ANNdist) ANN_SUM(box_dist,
		...
		if (child[ANN_LO] != KD_TRIVIAL)// enqueue if not trivial
			global.ANNprBoxPQ->insert(new_dist, child[ANN_LO]);
										// continue with closer child
		child[ANN_HI]->ann_pri_search(box_dist,visit,global);
	}
...
void ANNkd_leaf::ann_pri_search(ANNdist box_dist, ANNVisit& visit, ANNGlobal& global)
{
	...
	min_dist = global.ANNprPointMK->max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = global.ANNprPts[bkt[i]];			// first coord of next data point
		qq = global.ANNprQ;					// first coord of query point
		dist = 0;

		for(d = 0; d < global.ANNprDim; d++) {
		...
		if (d >= global.ANNprDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			global.ANNprPointMK->insert(dist, bkt[i]);
			min_dist = global.ANNprPointMK->max_key();
		}
	}
	ANN_LEAF(1)							// one more leaf node visited
	ANN_PTS(n_pts)						// increment points visited
	visit.ANNptsVisited += n_pts;		// increment number of points visited
}
