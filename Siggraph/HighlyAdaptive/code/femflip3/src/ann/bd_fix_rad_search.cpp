...
void ANNbd_shrink::ann_FR_search(ANNdist box_dist, ANNVisit &visit, ANNGlobal& global)
{
	...
	for (int i = 0; i < n_bnds; i++) {			// is query point in the box?
		if (bnds[i].out(global.ANNkdFRQ)) {			// outside this bounding side?
												// add to inner distance
			inner_dist = (ANNdist) ANN_SUM(inner_dist, bnds[i].dist(global.ANNkdFRQ));
		}
	}
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_FR_search(inner_dist,visit,global);// search inner child first
		child[ANN_OUT]->ann_FR_search(box_dist,visit,global);// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_FR_search(box_dist,visit,global);// search outer child first
		child[ANN_IN]->ann_FR_search(inner_dist,visit,global);// ...then outer child
	}
