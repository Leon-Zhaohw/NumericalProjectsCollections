...
// result = A * B
template<class T>
static bool multiply( SparseMatrix<T> &result, const SparseMatrix<T> &A, const SparseMatrix<T> &B ) {
	result.clear();
...
template<class T>
static void write_matlab1d( std::ostream &output, const std::vector<T> &v, const char *variable_name ) {
