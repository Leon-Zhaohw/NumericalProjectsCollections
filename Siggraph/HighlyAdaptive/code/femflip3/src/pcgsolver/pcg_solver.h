...
struct PCGSolver
{
	...
	bool solve(const SparseMatrix<T> &matrix, const std::vector<T> &rhs, std::vector<T> &result, T &residual_out, int &iterations_out, FLOAT64 &msec )
	{
		FLOAT64 msec0 = util3::getMilliseconds();
		...
		for(iteration=0; iteration<max_iterations; ++iteration){
			...
			if(residual_out<=tol) {
				...
				msec = util3::getMilliseconds()-msec0;
				return true;
			}
			apply_preconditioner(r, z);
