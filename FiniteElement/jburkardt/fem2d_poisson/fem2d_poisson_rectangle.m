...
%  Local:
%
%    real A(3*IB+1,NODE_NUM), the coefficient matrix.
%
%    real EH1, the H1 seminorm error.
%
%    real EL2, the L2 error.
%
...
%
%    real F(NODE_NUM), the right hand side.
%
%    integer IB, the half-bandwidth of the matrix.
%
%    integer NQ, the number of quadrature points used for assembly.
%
%    integer NODE_NUM, the number of nodes.
%
%    real NODE_XY(2,NODE_NUM), the nodes.
%
%    integer NQE, the number of quadrature points used for error
%    estimation.
%
%    integer NX, the number of points in the X direction.
%
%    integer NY, the number of points in the Y direction.
%
%    real WQ(NQ), quadrature weights.
%
%    real XL, XR, YB, YT, the X coordinates of
%    the left and right sides of the rectangle, and the Y coordinates
%    of the bottom and top of the rectangle.
%
%    real XQ(NQ,ELEMENT_NUM), YQ(NQ,ELEMENT_NUM), the
...
%
  ...
  fprintf ( 1, '  MATLAB/Octave version %d\n', version ( ) );
...
%    real NODE_XY(2,NODE_NUM), the nodes.
%
%    integer ELEMENT_ORDER, the number of local nodes per element.
%
%    integer ELEMENT_NUM, the number of elements.
%
%    integer ELEMENT_NODE(ELEMENT_ORDER,ELEMENT_NUM);
...
%    ELEMENT_NODE(I,J) is the global index of local node I in element J.
%
%    integer NQ, the number of quadrature points used in assembly.
%
%    real WQ(NQ), quadrature weights.
%
%    real XQ(NQ,ELEMENT_NUM), YQ(NQ,ELEMENT_NUM), the
...
%  Output:
%
%    real A(3*IB+1,NODE_NUM), the NODE_NUM by NODE_NUM
...
%  Local:
%
%    real BB, BX, BY, the value of some basis function
%    and its first derivatives at a quadrature point.
%
%    real BBB, BBX, BBY, the value of another basis
...
%  Input:
%
%    integer NX, NY, controls the number of elements along the
...
%  Output:
%
%    real A(3*IB+1,NODE_NUM), has been adjusted for boundary conditions.
%
%    real F(NODE_NUM), has been adjusted for boundary conditions.
%
...
%    real NODE_XY(2,NODE_NUM), the nodes.
%
%    real F(NODE_NUM), the solution vector of the finite
%    element system.
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'COMPARE:\n' );
...
%    real NODE_XY(2,NODE_NUM), the nodes.
%
%    real F(NODE_NUM), the coefficients of the solution.
%
%    integer ELEMENT_NUM, the number of elements.
%
%    integer ELEMENT_ORDER, the number of nodes used to form one element.
%
%    integer NQE, the number of points in the quadrature rule.
...
%  Local:
%
%    real AR, the weight for a given quadrature point
%    in a given element.
%
%    real BB, BX, BY, a basis function and its first
...
%    real EL2, the L2 error.
%
%    real UEX, UEXX, UEXY, the exact solution and its first
%    derivatives evaluated at a particular quadrature point.
%
%    real UH, UHX, UHY, the computed solution and its first
%    derivatives evaluated at a particular quadrature point.
%
%    real WQE(NQE), stores the quadrature weights.
%
%    real X, Y, the coordinates of a particular
%    quadrature point.
%
%    real XQE(NQE), YQE(NQE), stores the location
...
%  Input:
%
%    real X, Y, the (global) coordinates of the point
%    at which the basis function is to be evaluated.
%
%    integer ELEMENT, the index of the element which contains the point.
%
%    integer INODE, the local index (between 1 and 6) that
...
%  Output:
%
%    real B, DBDX, DBDY, the value of the basis function
...
%  Output:
%
%    real WQ(3), quadrature weights.
%
%    real XQ(3,ELEMENT_NUM), YQ(3,ELEMENT_NUM), the X and Y
%    coordinates of the quadrature points in each element.
%
  wq = zeros ( 3, 1 );
...
%    ELEMENT_NODE(I,J) is the global index of local node I in element J.
%
%    integer ELEMENT, the index of the element for which the quadrature
...
%  Output:
%
%    real WQE(13), the quadrature weights.
%
%    real XQE(NQE), YQE(NQE), the X and Y coordinates
...
%  Output:
%
%    real VALUE, the value of the right
...
%  Input:
%
%    integer NX, NY, the number of elements in the X and
%    Y direction.
%
%    integer NODE_NUM, the number of nodes.
%
