...
def reliability(x):
    ...
    stochastic_model.addVariable( Normal('X5',thk_p,0.03*thk_p) )  #0.03
...
class Test_Problem(nsga2.Problem):
    ...
    def __init__(self, offset=0):
        #thk_w,thk_cr, thk_p,ths_w, ths_cr, Tr_thk, Ts_thk, COV1, COV2, COV3, COV4
    ...
    def constraint(self, ind, metamodel=None):
        ...
        def rel(x):
            ...
            if x[1]>=0.15+delta2:
                ...
                p[3] = 0.0
            cp = 1000000.0
            return reliability([thk_w,thk_cr, thk_p,ths_w, ths_cr, Tr_thk, Ts_thk,x[0],x[1]])+cp*sum(p)

            
        w_t = optimize.fmin_powell(rel, [0.15,0.15], args=(), xtol=0.001, ftol=0.001, maxiter=None, maxfun=None, full_output=0, disp=0, retall=0, callback=None, direc=None)            
        index_w = rel(w_t)
        print index_w
        beta_w = 3.0
        if index_w>= beta_w:
...
def run():
    ...
    myOpt.run('CFRP_opt.db', 'CFRP_test', 1024, 100, 100)  
