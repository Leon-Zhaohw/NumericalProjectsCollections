...
public:    
	...
	PYTHON() int size() const { return mData.size(); }
	...
	PYTHON() void transformFrom(ParticleSystem<S> &p_old, const Vec3 &from_old, const Vec3 &to_old, const Vec3 &from_new, const Vec3 &to_new);
	...
	PYTHON() void clear();
	PYTHON() void killRegion(const FlagGrid& flags, const int target);

	//! if particle is stype and in cflag cell, set ptype as mark
	PYTHON() void setType(ParticleDataImpl<int> &ptype, const int mark, const int stype, const FlagGrid &flags, const int cflag);

	//! Advect particle in grid velocity field
	PYTHON() void advectInGrid(
	...
	PYTHON() void advect(const ParticleDataImpl<Vec3> &vel, const ParticleDataImpl<int> *ptype=NULL, const int exclude=0);
	PYTHON() void updateVelocity(ParticleDataImpl<Vec3> &vel, const Vec3 &a, const Real dt, const ParticleDataImpl<int> *ptype=NULL, const int exclude=0) const;
	PYTHON() void updateVelocityFromDeltaPos(ParticleDataImpl<Vec3> &vel, const ParticleDataImpl<Vec3> &x_prev, const Real dt, const ParticleDataImpl<int> *ptype=NULL, const int exclude=0) const;

	PYTHON() void getAtFromFlagGrid(
		ParticleDataImpl<int> &v, const FlagGrid &flags, const Vec3 offset=Vec3(0.),
		const ParticleDataImpl<int> *ptype=NULL, const int exclude=0) const;
	PYTHON() void getInterpolatedFromMACGrid(
		ParticleDataImpl<Vec3> &v, const MACGrid &grid, const Vec3 offset=Vec3(0.),
		const ParticleDataImpl<int> *ptype=NULL, const int exclude=0) const;
	PYTHON() void getInterpolatedFromGrid(
		ParticleDataImpl<Real> &v, const Grid<Real> &grid, const Vec3 offset=Vec3(0.),
		const ParticleDataImpl<int> *ptype=NULL, const int exclude=0) const;
	PYTHON() void getInterpolatedFromVecGrid(
		ParticleDataImpl<Vec3> &v, const Grid<Vec3> &grid, const Vec3 offset=Vec3(0.),
		const ParticleDataImpl<int> *ptype=NULL, const int exclude=0) const;
...
public:
	...
	//! file io
	PYTHON() void save(std::string name) const;
	...
	// for Blender
	PYTHON() void writeParticlesBobj(const std::string& name, const ParticleDataImpl<int> *pT=NULL, const int exclude=0) const;
...
public:
	...
	PYTHON() void setConstPart(const T& s, const ParticleDataImpl<int>& t, const int itype);
	...
	PYTHON() void invMultConst(const T& s); // NOTE: Caution! This means s/mData[i], not mData[i]/s
	...
	PYTHON() Real getMaxAbsValue() const;
	PYTHON() Real getMaxValue() const;
	PYTHON() Real getMinValue() const;
...
void KnSetType(ParticleDataImpl<int> &ptype, const ParticleSystem<S> &part, const int mark, const int stype, const FlagGrid &flags, const int cflag) {
...
std::vector<Vec3> GridAdvectKernel (std::vector<S>& p, const MACGrid& vel, const FlagGrid& flags, const Real dt,
...
KERNEL(pts) template<class S>
void KnAdvect(ParticleSystem<S> &p, const ParticleDataImpl<Vec3> &v, const Real dt, const ParticleDataImpl<int> *ptype, const int exclude) {
...
KERNEL(pts) template<class S>
void KnUpdateVelocityFromDeltaPos(const ParticleSystem<S> &p, ParticleDataImpl<Vec3> &v, const ParticleDataImpl<Vec3> &x_prev, const Real over_dt, const ParticleDataImpl<int> *ptype, const int exclude) {
	if(ptype && ((*ptype)[idx] & exclude)) return;
...
KERNEL(pts) template<class S>
void KnGetAtFromFlagGrid(
	const ParticleSystem<S> &p, ParticleDataImpl<int> &v, const FlagGrid &flags, const Vec3 offset,
	const ParticleDataImpl<int> *ptype, const int exclude) {
	if(ptype && ((*ptype)[idx] & exclude)) return;
...
KERNEL(pts) template<class S>
void KnGetInterpolatedFromMACGrid(
	const ParticleSystem<S> &p, ParticleDataImpl<Vec3> &v, const MACGrid &grid, const Vec3 offset,
	const ParticleDataImpl<int> *ptype, const int exclude) {
	if(ptype && ((*ptype)[idx] & exclude)) return;
	v[idx] = grid.getInterpolated(p[idx].pos + offset);
}

KERNEL(pts) template<class S, class T>
void KnGetInterpolatedFromGrid(
	const ParticleSystem<S> &p, ParticleDataImpl<T> &v, const Grid<T> &grid, const Vec3 offset,
	const ParticleDataImpl<int> *ptype, const int exclude) {
	if(ptype && ((*ptype)[idx] & exclude)) return;
	v[idx] = grid.getInterpolated(p[idx].pos + offset);
}
...
int cntPts(const ParticleDataImpl<int> &t, const int itype);
