...
inline static Real ghostFluidSurfTensHelper(const IndexInt idx, const int offset, const Grid<Real> &phi, const Grid<Real> &curv, const Real surfTens, const Real gfClamp);
...
void MakeRhs(
	...
	const Grid<Real> *phi, const Grid<Real> *curv, const Grid<Real> *perCellCorr, const MACGrid *fractions,
	const Real surfTens, const Real gfClamp)
{
	...
	if(phi && curv) {
		...
		if(flags.isEmpty(i-1,j,k))         set += ghostFluidSurfTensHelper(idx, -X, *phi, *curv, surfTens, gfClamp);
		if(flags.isEmpty(i+1,j,k))	   set += ghostFluidSurfTensHelper(idx, +X, *phi, *curv, surfTens, gfClamp);
		if(flags.isEmpty(i,j-1,k))	   set += ghostFluidSurfTensHelper(idx, -Y, *phi, *curv, surfTens, gfClamp);
		if(flags.isEmpty(i,j+1,k))	   set += ghostFluidSurfTensHelper(idx, +Y, *phi, *curv, surfTens, gfClamp);
		if(vel.is3D()) {
			if(flags.isEmpty(i,j,k-1)) set += ghostFluidSurfTensHelper(idx, -Z, *phi, *curv, surfTens, gfClamp);
			if(flags.isEmpty(i,j,k+1)) set += ghostFluidSurfTensHelper(idx, +Z, *phi, *curv, surfTens, gfClamp);
		}
	}
...
KERNEL(bnd=1)
void CorrectVelocityGhostFluid(
	MACGrid &vel, const FlagGrid &flags, const Grid<Real> &pressure, const Grid<Real> &phi, const Grid<Real> *curv, const Real surfTens, const Real gfClamp)
{
	...
	if(curv) {
		if(flags.isFluid(idx))	{
			if(flags.isEmpty(i-1,j,k))		   vel[idx].x += ghostFluidSurfTensHelper(idx, -X, phi, *curv, surfTens, gfClamp);
			if(flags.isEmpty(i,j-1,k))		   vel[idx].y += ghostFluidSurfTensHelper(idx, -Y, phi, *curv, surfTens, gfClamp);
			if(flags.is3D() && flags.isEmpty(i,j,k-1)) vel[idx].z += ghostFluidSurfTensHelper(idx, -Z, phi, *curv, surfTens, gfClamp);
		} else if (flags.isEmpty(idx)&&!flags.isOutflow(idx)) { // do not change velocities in outflow cells
			vel[idx].x -= (flags.isFluid(i-1,j,k)) ? ghostFluidSurfTensHelper(idx-X, +X, phi, *curv, surfTens, gfClamp) : 0.f;
			vel[idx].y -= (flags.isFluid(i,j-1,k)) ? ghostFluidSurfTensHelper(idx-Y, +Y, phi, *curv, surfTens, gfClamp) : 0.f;
			if(flags.is3D()) {
				vel[idx].z -= (flags.isFluid(i,j,k-1)) ? ghostFluidSurfTensHelper(idx-Z, +Z, phi, *curv, surfTens, gfClamp) : 0.f;
			}
		}
	}
}
...
KERNEL(bnd=1)
void ReplaceClampedGhostFluidVels(
	MACGrid &vel, const FlagGrid &flags, const Grid<Real> &pressure, const Grid<Real> &phi, const Real gfClamp)
{
	const IndexInt idx = flags.index(i,j,k);
	const IndexInt X   = flags.getStrideX(), Y = flags.getStrideY(), Z = flags.getStrideZ();
...
PYTHON()
void solvePressure(
	MACGrid &vel, Grid<Real> &pressure, FlagGrid &flags,
	const Real cgAccuracy = 1e-3,
	const Grid<Real> *phi = NULL,
	const Grid<Real> *curv = NULL,
	const Grid<Real> *perCellCorr = NULL,
	const MACGrid *fractions = NULL,
	const Real surfTens = 0.073,
	const Real gfClamp = 1e-04,
	const Real cgMaxIterFac = 1.5,
	const bool precondition = true, // Deprecated, use preconditioner instead
	...
	Grid<Real> *retRhs = NULL)
{
	...
	MakeRhs kernMakeRhs(flags, rhs, vel, phi, curv, perCellCorr, fractions, surfTens, gfClamp);
	...
	if(phi) {
		CorrectVelocityGhostFluid(vel, flags, pressure, *phi, curv, surfTens, gfClamp);
