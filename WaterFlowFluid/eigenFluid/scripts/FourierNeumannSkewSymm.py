...
problem = de.IVP(domain, variables=['p','u','v','u2','v2','uv'])
...
problem.meta['u2']['x']['parity'] = 1
problem.meta['v2']['x']['parity'] = 1
problem.meta['uv']['x']['parity'] = -1

#N-S equation 
problem.add_equation("dt(u) + dx(p) = - 0.5*(u*dx(u) + v*dy(u)) - 0.5*(dx(u2) + dy(uv))")
problem.add_equation("dt(v) + dy(p) = - 0.5*(u*dx(v) + v*dy(v)) - 0.5*(dx(uv) + dy(v2))")
problem.add_equation("u2 = u*u")
problem.add_equation("v2 = v*v")
problem.add_equation("uv = u*v")
