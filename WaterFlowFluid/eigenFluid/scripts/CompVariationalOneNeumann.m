% Numerically compute the tensor for one Neumann wall for variational tensor.
...
for k = 0:(num_basis - 1)
    for j = 0:(num_basis - 1)
        for i = 0:(num_basis - 1)
            ...
            entry = TensorEntVarDirich(i1-0.5 ,i2,j1-0.5,j2,k1-0.5,k2);
            %Matlab begin with 1.
            C(i+1,j+1,k+1) = entry;
            if (abs(entry) > 1e-10)
                total = total + 1;
...
fd = fopen('tensor_var_oneNeumann.dat','w');
fwrite(fd,C,'double');
fclose(fd);