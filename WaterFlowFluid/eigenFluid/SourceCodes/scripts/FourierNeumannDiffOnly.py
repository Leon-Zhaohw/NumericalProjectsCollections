...
problem = de.IVP(domain, variables=['p','u','v','uy','vy'])
...
problem.meta['vy']['x']['parity'] = 1
#N-S equation 
problem.add_equation("dt(u) + dx(p) - 1/Re*(dx(dx(u)) + dy(uy)) = 0")
problem.add_equation("dt(v) + dy(p) - 1/Re*(dx(dx(v)) + dy(vy)) = 0")
