...
void SixNeumannBasisSet3D::FillBasisNumerical(
      const Eigen::VectorXd& coefficients, VECTOR3_FIELD_3D* field) {
  if (coefficients.size() != basis_dim_) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << " The number of coefficients" << " does not equal to number of basis." << std::endl; exit(0);}
  if (! basis_allocated_) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  for (int i = 0; i < basis_dim_; i++) {
    all_basis_[i].DiscretizeAdd(coefficients[i], field);
  }
}

void SixNeumannBasisSet3D::ComputeBasisWeights(Eigen::VectorXd* weights, const Real weightMultiplierC) {
  std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Not implemented." << std::endl; exit(0);
}

double SixNeumannBasisSet3D::ComputeBasisAt(const VEC3I& pos, const int basis_idx, const int mode) const {
  if (basis_idx < 0 || basis_idx >= basis_dim_) {
    std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "basis index is out of range: " << basis_idx << std::endl; exit(0);
  }
...
int SixNeumannBasisSet3D::AllocateAllBasis() {
  ...
  for (int i = 0; i < waveNs.size(); i++) {
    ...
    SixNeumannBasis3D basis(K_[0], K_[1], K_[2], coef(0), coef(1), coef(2));
...
Eigen::Vector3d SixNeumannBasisSet3D::GenerateCoefficients(const int k1, const int k2, const int k3) {
  int num_zero = 0;
  if (k1 == 0) num_zero ++;
  if (k2 == 0) num_zero ++;
  if (k3 == 0) num_zero ++;
  if (num_zero == 3 || num_zero == 2) {   // zero mode.
        ...
        case 3:  // random
            {
              ...
              if (which == 0) { // rand on y,z
                 ...
                 return Eigen::Vector3d(r1, r2, C);
                   ...
                   std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "WTF.." << std::endl; exit(0);
               }
            }
          break;
        case 4:  // uniform.
            {
              const double A = k1*k2 + k1*k3 -(k2*k2+k3*k3);
              const double B = k1*k2 + k2*k3 -(k1*k1 + k3*k3);
              const double C = k1*k3 + k2*k3 -(k1*k1+k2*k2);
              return Eigen::Vector3d(A,B,C);
            }
          break;
        default:
          std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "...." << std::endl; exit(0);
          break;
    }
  }
}
...
void SixNeumannBasisSet3D::AllocateAllBasis(const std::vector<SixNeumannBasis3D>& basis) {
...
void SixNeumannBasisSet3D::Allocate_HighFreqWaveNumber(const int high_num, const VEC3I& lowNS, const float Ydx,
                                   const float Zdx, std::vector<VEC3I>* waveNs) {
   std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Not implemented." << std::endl; exit(0);
}

void SixNeumannBasisSet3D::WriteBasis(std::ofstream& out) {
  if (!out.is_open()) {
    std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Cannot write to file stream" << std::endl; exit(0);
  }
  // The type number for this one is 6.
  int basis_type_ = 6;
  if (basis_dim_ != all_basis_.size()) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " << std::endl; exit(0);}
...
int SixNeumannBasisSet3D::ReadBasis(std::ifstream& infile) {
  if (! !basis_allocated_) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis already allocated." << std::endl; exit(0);}
  
  if (!infile.is_open()) {
    std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Cannot read from stream." << std::endl; exit(0);
  }
  int read_type = -1;
  int read_dim = -1;
  ...
  if (read_type != 6) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "expect Six Neumann wall type." << std::endl; exit(0);}
  ...
  for (int i = 0; i < read_dim; i++) {
    ...
    SixNeumannBasis3D basis (K_[0], K_[1], K_[2], A, B, C);
    all_basis_.push_back(basis);
  }
...
void SixNeumannBasisSet3D::FillVariationalTensor(std::vector<Adv_Tensor_Type> *Adv_tensor) {
  ...
  for (int k = 0; k < basis_dim_; k++) {
    ...
    for (int j = 0; j < basis_dim_; j++) {
      for (int i = 0; i < basis_dim_; i++) {
        //
        const SixNeumannBasis3D& basis_i = all_basis_[i];
        const SixNeumannBasis3D& basis_j = all_basis_[j];
        const SixNeumannBasis3D& basis_k = all_basis_[k];
...
// P_x in doc.
double SixNeumannBasisSet3D::ComputeXIntegral(
    const SixNeumannBasis3D& basis_i, const SixNeumannBasis3D& basis_j, 
  ...
  // CoefL = Ci*i_2 - Bi*i_3 
  const double CoefL = Ci*I_[1] - Bi*I_[2];
  if (CoefL == 0) return 0;
  // CoefR = \sum_{i = 0}^7 c_i^x \int_{\Omega} \xi^1(i_1, i_2, i_3) \xi^6(...) d\Omega SCCCSS
  double CoefR = 0;
  // +++
  CoefR += (-BkCj + BjCk)*ComputeIntegralSCCCSS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]+K_[2]);
  // ++-
  CoefR +=  (BkCj + BjCk)*ComputeIntegralSCCCSS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]-K_[2]);
  // +-+
  CoefR += (-BkCj - BjCk)*ComputeIntegralSCCCSS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]-K_[1], J_[2]+K_[2]);
...
// P_y in doc.
double SixNeumannBasisSet3D::ComputeYIntegral(
    const SixNeumannBasis3D& basis_i, const SixNeumannBasis3D& basis_j,
    const SixNeumannBasis3D& basis_k) {
  ...
  if (CoefL == 0) return 0;
  // CoefR = \sum_{i = 0}^7 c_i^y \int_{\Omega} \xi^2(i_1, i_2, i_3) \xi^5(...) d\Omega CSCSCS
  double CoefR = 0;
  // +++
  CoefR += (-AjCk + AkCj)*ComputeIntegralCSCSCS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]+K_[2]);
  // ++-
  CoefR += (-AjCk - AkCj)*ComputeIntegralCSCSCS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]-K_[2]);
  // +-+
  CoefR +=  (AjCk - AkCj)*ComputeIntegralCSCSCS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]-K_[1], J_[2]+K_[2]);
...
// P_z in doc.
double SixNeumannBasisSet3D::ComputeZIntegral(
    const SixNeumannBasis3D& basis_i, const SixNeumannBasis3D& basis_j,
    const SixNeumannBasis3D& basis_k) {
  ...
  if (CoefL == 0) return 0;
  // CoefR = \sum_{i = 0}^7 c_i^z \int_{\Omega} \xi^3(i_1, i_2, i_3) \xi^4(...) d\Omega CCSSSC
  double CoefR = 0;
  // +++
  CoefR += (-AkBj + AjBk)*ComputeIntegralCCSSSC(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]+K_[2]);
  // ++-
  CoefR +=  (AkBj - AjBk)*ComputeIntegralCCSSSC(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]-K_[2]);
  // +-+
  CoefR +=  (AkBj + AjBk)*ComputeIntegralCCSSSC(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]-K_[1], J_[2]+K_[2]);
...
void SixNeumannBasisSet3D::ComputeEigenValues(Eigen::VectorXd* eigenValue) {
  if (! basis_allocated_) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  for (int i = 0; i < basis_dim_; i++) {
    VEC3I K_ = all_basis_[i].GetWaveNum();
...
void SixNeumannBasisSet3D::InverseTramsformToVelocity(
      const Eigen::VectorXd& coefficients, VECTOR3_FIELD_3D* field) {
  if (! basis_allocated_) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  if (coefficients.size() != all_basis_.size()) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " << "The basis size and " << "coefficients size must be equal." << std::endl; exit(0);}
  field->clear();
  ClearTemp();
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    if (K_[1] < 1 || K_[2] < 1) {  // vx is zero.
      continue;
    }
    // Process x field. x: REDFT01, y: RODFT01 z: RODFT01. 
    temp_[K_[0] + (K_[1] - 1)*xRes_ + (K_[2] - 1)*SlabSize_] = coefficients[i] * C_x;
  }
  fftw_plan plan_x_3D;
  plan_x_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_, temp_, temp_,
                               FFTW_RODFT01, FFTW_RODFT01, FFTW_REDFT01, FFTW_MEASURE);
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    if (K_[0] < 1 || K_[2] < 1) {  // vy = 0.
      continue;
    }
    // Process y field. x: RODFT01, y: REDFT01 z: RODFT01.
    temp_[K_[0] - 1 + K_[1]*xRes_ + (K_[2] - 1)*SlabSize_] = coefficients[i] * C_y;
  }
  fftw_plan plan_y_3D;
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    if (K_[0] < 1 || K_[1] < 1) {  // vz = 0.
      continue;
    }
    // Process z field. x: RODFT01, y: RODFT01 z: REDFT01.
    temp_[K_[0] - 1 + (K_[1] - 1)*xRes_ + K_[2]*SlabSize_] = coefficients[i] * C_z;
  }
  fftw_plan plan_z_3D;
...
void SixNeumannBasisSet3D::ForwardTransformtoFrequency(
      const VECTOR3_FIELD_3D& field, Eigen::VectorXd* coefficients) {
  if (! basis_allocated_) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  if (coefficients->size() != basis_dim_) {std::cout << "six_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << std::endl; exit(0);}
  ...
  fftw_plan plan_x_3D;
  // x: REDFT10, y: RODFT10 z: RODFT10.
  plan_x_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_, temp_, temp_,
                               FFTW_RODFT10 , FFTW_RODFT10, FFTW_REDFT10, FFTW_MEASURE);
  fftw_execute(plan_x_3D);
  fftw_destroy_plan(plan_x_3D);
  for (int i = 0; i < basis_dim_; i++) {
    // Get wavenumber.
    ...
    (*coefficients)[i] += temp_[K_[0] + (K_[1] - 1)*xRes_ +
                                (K_[2] - 1)*SlabSize_]*invTotalSize_*PI_CUBE*0.125*C_x;
  }
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    if (K_[0] < 1 || K_[2] < 1) {  // vy is zero.
      continue;
    }
    (*coefficients)[i] += temp_[K_[0] - 1 + K_[1]*xRes_ + 
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    if (K_[0] < 1 || K_[1] < 1) {  // vz is zero.
      continue;
    }
    (*coefficients)[i] += temp_[K_[0] - 1 + (K_[1] - 1)*xRes_ + 
                                K_[2]*SlabSize_]*invTotalSize_*PI_CUBE*0.125*C_z;
  }
}
