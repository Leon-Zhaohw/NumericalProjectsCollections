...
void FourNeumannBasisSet3D::FillBasisNumerical(
    const Eigen::VectorXd& coefficients, VECTOR3_FIELD_3D* field) {
  if (coefficients.size() != basis_dim_) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << " The number of coefficients" << " does not equal to number of basis." << std::endl; exit(0);}
  if (! basis_allocated_) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  for (int i = 0; i < basis_dim_; i++) {
    all_basis_[i].DiscretizeAdd(coefficients[i], field);
  }
}

void FourNeumannBasisSet3D::ComputeBasisWeights(Eigen::VectorXd* weights, const Real weightMultiplierC) {
  std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Not implemented." << std::endl; exit(0);
}

// initialize the basis vector using externally allocated basis.
void FourNeumannBasisSet3D::AllocateAllBasis(const std::vector<FourNeumannBasis3D>& basis) {
...
int FourNeumannBasisSet3D::AllocateAllBasis() {
  ...
  for (int i = 0; i < waveNs.size(); i++) {
    ...
    FourNeumannBasis3D basis(K_[0], K_[1], K_[2], coef(0), coef(1), coef(2));
...
// Randomized for now...
Eigen::Vector3d FourNeumannBasisSet3D::GenerateCoefficients(const int k1, const int k2, const int k3) {
    int num_zero = 0;
  if (k1 == 0) num_zero ++;
  if (k2 == 0) num_zero ++;
  if (k3 == 0) num_zero ++;
  if (num_zero == 3) {   // zero mode.
    return Eigen::Vector3d(0,0,0);
  }
  if (num_zero == 2) {
    if (k1 == 0 && k3 == 0) { // zero mode
      return Eigen::Vector3d(0,0,0);
    } else if (k1 == 0 && k2 == 0) {   // only vx != 0
      return Eigen::Vector3d(1,0,0);
    } else {   // k2_ == 0 && k3_ == 0  only vz !=0
  ...
  if (num_zero == 0) {
    switch (int_type_const_strategy_) {
        case 0:  // principle_x
          ...
          break;
        case 1:  // principle_y
          return Eigen::Vector3d(k2*k1, (k1*k1+k3*k3), k2*k3);
          break;
        case 2:  // principle_z
        ...
        case 3:  // random
            {
              ...
              if (which == 0) { // rand on y,z
                 ...
                 return Eigen::Vector3d(r1, r2, C);
                   ...
                   std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "WTF.." << std::endl; exit(0);
              }
            }
          break;
        case 4: // uniform
            {
              const double A = k2*k2+k3*k3 - k2*k1 - k1*k3;
          ...
          break;
        default:
          std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "...." << std::endl; exit(0);
          break;
    }
  }
}

double FourNeumannBasisSet3D::ComputeBasisAt(const VEC3I& pos, const int basis_idx, const int mode) const {
  if (basis_idx < 0 || basis_idx >= basis_dim_) {
    std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "basis index is out of range: " << basis_idx << std::endl; exit(0);
  }
  return all_basis_[basis_idx].ComputeBasisAt(pos[0], pos[1], pos[2], mode, dxyz_);
}
  
void FourNeumannBasisSet3D::ComputeEigenValues(Eigen::VectorXd* eigenValue) {
  if (! basis_allocated_) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  for (int i = 0; i < basis_dim_; i++) {
...
void FourNeumannBasisSet3D::WriteBasis(std::ofstream& out) {
  if (!out.is_open()) {
    std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Cannot write to file stream" << std::endl; exit(0);
  }
  // The type number for this one is 4.
  int basis_type_ = 4;
  if (basis_dim_ != all_basis_.size()) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " << std::endl; exit(0);}
...
int FourNeumannBasisSet3D::ReadBasis(std::ifstream& infile) {
  if (! !basis_allocated_) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis already allocated." << std::endl; exit(0);}
  
  if (!infile.is_open()) {
    std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Cannot read from stream." << std::endl; exit(0);
  }
  int read_type = -1;
  int read_dim = -1;
  ...
  if (read_type != 4) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "expect Four Neumann wall type." << std::endl; exit(0);}
  ...
  for (int i = 0; i < read_dim; i++) {
    ...
    FourNeumannBasis3D basis (K_[0], K_[1], K_[2], A, B, C);
    all_basis_.push_back(basis);
  }
...
void FourNeumannBasisSet3D::FillVariationalTensor(std::vector<Adv_Tensor_Type> *Adv_tensor) {
  if (! basis_allocated_) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  ...
  for (int k = 0; k < basis_dim_; k++) {
    ...
    for (int j = 0; j < basis_dim_; j++) {
      for (int i = 0; i < basis_dim_; i++) {
        //
        const FourNeumannBasis3D& basis_i = all_basis_[i];
        const FourNeumannBasis3D& basis_j = all_basis_[j];
        const FourNeumannBasis3D& basis_k = all_basis_[k];
...
// P_x in doc.
double FourNeumannBasisSet3D::ComputeXIntegral(const FourNeumannBasis3D& basis_i,
                                               const FourNeumannBasis3D& basis_j, const FourNeumannBasis3D& basis_k) {
  ...
  if (CoefL == 0) return 0;
  // CoefR = \sum_{i = 0}^7 c_i^x \int_{\Omega} \xi^4(i_1, i_2, i_3) \xi^6(...) d\Omega SSCCSS
  double CoefR = 0;
  // +++
  CoefR += (-BkCj + BjCk)*ComputeIntegralSSCCSS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]+K_[2]);
  // ++-
  CoefR +=  (BkCj + BjCk)*ComputeIntegralSSCCSS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]-K_[2]);
  // +-+
  CoefR +=  (BkCj + BjCk)*ComputeIntegralSSCCSS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]-K_[1], J_[2]+K_[2]);
...
// P_y in doc.
double FourNeumannBasisSet3D::ComputeYIntegral(const FourNeumannBasis3D& basis_i,
                                               const FourNeumannBasis3D& basis_j, const FourNeumannBasis3D& basis_k) {
  const VEC3I I_ = basis_i.GetWaveNum();
  const VEC3I J_ = basis_j.GetWaveNum();
  const VEC3I K_ = basis_k.GetWaveNum();
  ...
  // (A_i i_3 - C_i i_1)
  const double CoefL = Ai*I_[2] - Ci*I_[0];
  if (CoefL == 0) return 0;
  // CoefR = \sum_{i = 0}^7 c_i^y \int_{\Omega} \xi^0(i_1, i_2, i_3) \xi^5(...) d\Omega CCCSCS
  double CoefR = 0;
  // +++
  CoefR +=  (AjCk - AkCj)*ComputeIntegralCCCSCS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]+K_[2]);
  // ++-
  CoefR +=  (AjCk + AkCj)*ComputeIntegralCCCSCS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]-K_[2]);
  // +-+
  CoefR +=  (AjCk - AkCj)*ComputeIntegralCCCSCS(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]-K_[1], J_[2]+K_[2]);
...
// P_z in doc.
double FourNeumannBasisSet3D::ComputeZIntegral(const FourNeumannBasis3D& basis_i,
                                               const FourNeumannBasis3D& basis_j, const FourNeumannBasis3D& basis_k) {
  const VEC3I I_ = basis_i.GetWaveNum();
  const VEC3I J_ = basis_j.GetWaveNum();
  const VEC3I K_ = basis_k.GetWaveNum();
  ...
  if (CoefL == 0) return 0;
  // CoefR = \sum_{i = 0}^7 c_i^z \int_{\Omega} \xi^6(i_1, i_2, i_3) \xi^4(...) d\Omega CSSSSC
  double CoefR = 0;
  // +++
  CoefR += (-AkBj + AjBk)*ComputeIntegralCSSSSC(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]+K_[2]);
  // ++-
  CoefR +=  (AkBj - AjBk)*ComputeIntegralCSSSSC(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]+K_[1], J_[2]-K_[2]);
  // +-+
  CoefR += (-AkBj - AjBk)*ComputeIntegralCSSSSC(I_[0], I_[1], I_[2], J_[0]+K_[0], J_[1]-K_[1], J_[2]+K_[2]);
...
void FourNeumannBasisSet3D::ForwardTransformtoFrequency(
    const VECTOR3_FIELD_3D& field, Eigen::VectorXd* coefficients) {
  if (! basis_allocated_) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  if (coefficients->size() != basis_dim_) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << std::endl; exit(0);}
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    (*coefficients)[i] += temp_[K_[0] + K_[1]*xRes_ + (K_[2]-1)*SlabSize_]*invTotalSize_*PI_CUBE*0.125*C_x;
  }
  ...
  fftw_plan plan_y_3D;
  // x: RODFT10, y: RODFT10 z: RODFT10.
  plan_y_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_, temp_, temp_,
                               FFTW_RODFT10 , FFTW_RODFT10, FFTW_RODFT10, FFTW_MEASURE);
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    if (K_[0] < 1 || K_[1] < 1 || K_[2] < 1) {  // vy is zero.
      continue;
    }
    (*coefficients)[i] += temp_[K_[0] - 1 + (K_[1] - 1)*xRes_ +
                           (K_[2] - 1)*SlabSize_]*invTotalSize_*PI_CUBE*0.125*C_y;
  }
  // vz...
  // Put the value into field.
  ClearTemp();
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    if (K_[0] < 1) {  // vz is zero.
      continue;
    }
    (*coefficients)[i] += temp_[K_[0] - 1 + K_[1]*xRes_ + K_[2]*SlabSize_]*invTotalSize_*PI_CUBE*0.125*C_z;
  }
}

void FourNeumannBasisSet3D::InverseTramsformToVelocity(
      const Eigen::VectorXd& coefficients, VECTOR3_FIELD_3D* field) {
  if (! basis_allocated_) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  if (coefficients.size() != all_basis_.size()) {std::cout << "four_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " << "The basis size and " << "coefficients size must be equal." << std::endl; exit(0);}
  field->clear();
  ClearTemp();
  // vx..
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    // Collect constant field.
    if (K_[2] < 1) {  // vx = 0.
      continue;
    }
    // Process x field. x: REDFT01, y: REDFT01 z: RODFT01.
    temp_[K_[0] + K_[1]*xRes_ + (K_[2] - 1)*SlabSize_] = coefficients[i] * C_x;
    //temp_[1 + xRes_ + SlabSize_] = 5;
  }
  fftw_plan plan_x_3D;
  plan_x_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_, temp_, temp_,
  ...
  fftw_execute(plan_x_3D);  
  fftw_destroy_plan(plan_x_3D);
  // Put the value back into field, add the constant mode.
  for (uint i = 0; i < TotalSize_; i++) {
     (*field)[i][0] = temp_[i];
  }
  ClearTemp();
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    if (K_[0] < 1 || K_[1] < 1 || K_[2] < 1) {  // vy = 0.
      continue;
    }
    // Process y field. x: RODFT01, y: RODFT01 z: RODFT01.
    temp_[K_[0] - 1 + (K_[1] - 1)*xRes_ + (K_[2]-1)*SlabSize_] = coefficients[i] * C_y;
  }
  fftw_plan plan_y_3D;
  plan_y_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_, temp_, temp_,
                               FFTW_RODFT01, FFTW_RODFT01, FFTW_RODFT01, FFTW_MEASURE);
  fftw_execute(plan_y_3D);  
  fftw_destroy_plan(plan_y_3D);
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    // Process z field. x: RODFT01, y: REDFT01 z: REDFT01.
    temp_[K_[0] - 1 + K_[1]*xRes_ + K_[2]*SlabSize_] = coefficients[i] * C_z;
  }
  fftw_plan plan_z_3D;
  plan_z_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_, temp_, temp_,
