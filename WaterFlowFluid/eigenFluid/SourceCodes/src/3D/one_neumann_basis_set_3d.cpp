...
void OneNeumannBasisSet3D::FillBasisNumerical(
  const Eigen::VectorXd& coefficients, VECTOR3_FIELD_3D* field) {
  if(coefficients.size() != basis_dim_) { std::cout << " The number of coefficients"
      << " does not equal to number of basis." << std::endl; exit(0);};
  if (! basis_allocated_) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  for (int i = 0; i < basis_dim_; i++) {
    all_basis_[i].DiscretizeAdd(coefficients[i], field);
  }
}

void OneNeumannBasisSet3D::ComputeBasisWeights(Eigen::VectorXd* weights, const Real weightMultiplierC) {
  std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Not implemented." << std::endl; exit(0);
}

double OneNeumannBasisSet3D::ComputeBasisAt(const VEC3I& pos, 
                                            const int basis_idx, const int mode) const {
  if (basis_idx < 0 || basis_idx >= basis_dim_) {
    std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "basis index is out of range: " << basis_idx << std::endl; exit(0);
  }
  return all_basis_[basis_idx].ComputeBasisAt(pos[0], pos[1], pos[2], mode, dxyz_);
}

void OneNeumannBasisSet3D::AllocateAllBasis(const std::vector<OneNeumannBasis3D>& basis) {
...
Eigen::Vector3d OneNeumannBasisSet3D::GenerateCoefficients(
  ...
  if (num_zero == 1) {
    ...
    if (k2 == 0) {  // k1 >= 1
      return Eigen::Vector3d(-k3, 0, k1-0.5);
      //return Eigen::Vector3d(0,0,0);
    }
    if (k3 == 0) {
      return Eigen::Vector3d(k2, -(k1-0.5), 0);
      //return Eigen::Vector3d(0,0,0);
    }
  }
  
  if (num_zero == 0) {  // k1 >= 1, k2 >= 1, k3 >= 1
    switch (int_type_const_strategy_) {
      case 0:  // principle_x
        return Eigen::Vector3d(-(k2*k2+k3*k3), (k1-0.5)*k2, (k1-0.5)*k3);
        break;
      case 1:  // principle_y
        return Eigen::Vector3d((k1-0.5)*k2, -((k1-0.5)*(k1-0.5) + k3*k3), k2*k3);
        break;
      case 2:  // principle_z
        return Eigen::Vector3d((k1-0.5)*k3, k2*k3, -((k1-0.5)*(k1-0.5)+k2*k2));
        break;
      case 3:  // Random
          {
            ...
            if (which == 0) { // rand on y,z
              const double A = (- r1*k2 - r2*k3) / (static_cast<double>(k1) - 0.5);
              return Eigen::Vector3d(A, r1, r2);
            } else if (which == 1) { // rand on x, z
              const double B = (-r1*(k1 - 0.5) - r2*k3) / static_cast<double>(k2);
              return Eigen::Vector3d(r1, B, r2);
            } else if (which == 2) {  // rand on x,y
              const double C = (-r1*(k1 - 0.5) - r2*k2) / static_cast<double>(k3);
              return Eigen::Vector3d(r1, r2, C);
            } else {
              std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "WTF.." << std::endl; exit(0);
            }
          }
        break;
      case 4:  // Uniform
          {
            const double A = -(k2*k2+k3*k3) + (k1-0.5)*k2 + (k1-0.5)*k3;
            const double B = (k1-0.5)*k2 - ((k1-0.5)*(k1-0.5) + k3*k3) + k2*k3;
            const double C = (k1-0.5)*k3 + k2*k3 -((k1-0.5)*(k1-0.5)+k2*k2);
            return Eigen::Vector3d(A, B, C);
          }
        break;
      default:
        std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "..." << std::endl; exit(0);
        break;
    }
  }
}

void OneNeumannBasisSet3D::ComputeEigenValues(Eigen::VectorXd* eigenValue) {
  if (! basis_allocated_) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  for (int i = 0; i < basis_dim_; i++) {
    VEC3I K_ = all_basis_[i].GetWaveNum();
    (*eigenValue)(i) = (K_[0]-0.5)*(K_[0]-0.5) + K_[1]*K_[1] + K_[2]*K_[2];
  }
}

int OneNeumannBasisSet3D::AllocateAllBasis() {
...
// Allocate more basis along longer dimension.
  ...
  float n_ = pow(static_cast<float>(des_basis_dim_)/(Ydx*Zdx), 1.0 / 3.0);
  ...
  for (int i = 0; i < waveNs.size(); i++) {
    ...
    OneNeumannBasis3D basis(K_[0], K_[1], K_[2], coef(0), coef(1), coef(2));
...
int OneNeumannBasisSet3D::ReadBasis(std::ifstream& infile) {
  if (! !basis_allocated_) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis already allocated." << std::endl; exit(0);}
  
  if (!infile.is_open()) {
    std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Cannot read from stream." << std::endl; exit(0);
  }
  int read_type = -1;
  int read_dim = -1;
  ...
  if (read_type != 1) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "expect one neumann type." << std::endl; exit(0);}
  ...
  for (int i = 0; i < read_dim; i++) {
    ...
    OneNeumannBasis3D basis (K_[0], K_[1], K_[2], A, B, C);
    all_basis_.push_back(basis);
  }
...
void OneNeumannBasisSet3D::WriteBasis(std::ofstream& out) {
  if (!out.is_open()) {
    std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " <<  "Cannot write to file stream" << std::endl; exit(0);
  }
  int basis_type_ = 1;
  if (basis_dim_ != all_basis_.size()) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: " << std::endl; exit(0);}
...
void OneNeumannBasisSet3D::FillVariationalTensor(std::vector<Adv_Tensor_Type> *Adv_tensor) {
  if (! basis_allocated_) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
...
#pragma omp parallel for
  for (int k = 0; k < basis_dim_; k++) {
    ...
    for (int j = 0; j < basis_dim_; j++) {
      for (int i = 0; i < basis_dim_; i++) {
        //
        const OneNeumannBasis3D& basis_i = all_basis_[i];
        const OneNeumannBasis3D& basis_j = all_basis_[j];
        const OneNeumannBasis3D& basis_k = all_basis_[k];
...
// P_x in doc.
double OneNeumannBasisSet3D::ComputeXIntegral(
  const OneNeumannBasis3D& basis_i, const OneNeumannBasis3D& basis_j,
  ...
  double CoefR = 0;
  // +++
  CoefR +=  (BkCj - BjCk)*ComputeIntegralCSSDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] + K_[1], J_[2] + K_[2]);
  // ++-
  CoefR +=  (BkCj + BjCk)*ComputeIntegralCSSDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] + K_[1], J_[2] - K_[2]);
  // +-+
  CoefR += -(BkCj + BjCk)*ComputeIntegralCSSDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] - K_[1], J_[2] + K_[2]);
...
// P_y in doc.
double OneNeumannBasisSet3D::ComputeYIntegral(
  const OneNeumannBasis3D& basis_i, const OneNeumannBasis3D& basis_j,
  const OneNeumannBasis3D& basis_k) {
  ...
  // CoefL = Ci*i_1 - Ai*i_3
  const double CoefL = (Ci*i0 - Ai*I_[2]);
  if (CoefL == 0) return 0;
  // CoefR = \sum_{i = 0}^7 c_i^y \int_{\Omega} \xi^5(i_1, i_2, i_3) \xi^5(...) d\Omega
  double CoefR = 0;
  // +++
  CoefR += (-AkCj + AjCk)*ComputeIntegralSCSDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] + K_[1], J_[2] + K_[2]);
  // ++-
  CoefR += (-AkCj - AjCk)*ComputeIntegralSCSDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] + K_[1], J_[2] - K_[2]);
  // +-+
  CoefR += (-AkCj + AjCk)*ComputeIntegralSCSDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] - K_[1], J_[2] + K_[2]);
...
// P_z in doc.
double OneNeumannBasisSet3D::ComputeZIntegral(
  const OneNeumannBasis3D& basis_i, const OneNeumannBasis3D& basis_j,
  const OneNeumannBasis3D& basis_k) {
  ...
  // CoefL = Ai*i_2 - Bi*i_1
  const double CoefL = (Ai*I_[1] - Bi*i0);
  if (CoefL == 0) return 0;
  // CoefR = \sum_{i = 0}^7 c_i^z \int_{\Omega} \xi^4(i_1, i_2, i_3) \xi^4(...) d\Omega
  double CoefR = 0;
  // +++
  CoefR +=  (AkBj - AjBk)*ComputeIntegralSSCDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] + K_[1], J_[2] + K_[2]);
  // ++-
  CoefR +=  (AkBj - AjBk)*ComputeIntegralSSCDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] + K_[1], J_[2] - K_[2]);
  // +-+
  CoefR +=  (AkBj + AjBk)*ComputeIntegralSSCDOUBLE(i0, I_[1], I_[2], j0 + k0, J_[1] - K_[1], J_[2] + K_[2]);
...
void OneNeumannBasisSet3D::ForwardTransformtoFrequency(
      const VECTOR3_FIELD_3D& field, Eigen::VectorXd* coefficients) {
  if (! basis_allocated_) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  if (coefficients->size() != basis_dim_) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << std::endl; exit(0);}
  coefficients->setZero();
  ClearTemp();
  // vx...
  // Put the value into field.
  uint index = 0;
  for (uint k = 0; k < zRes_; k++) {
    for (uint j = 0; j < yRes_; j++) {
      for (uint i = 0; i < xRes_; i++) {
        uint tempid = i + j*2*xRes_ + k*2*SlabSize_;
        temp_[tempid] = field[index][0]; 
        index ++;
      }
    }
  }
  ...
  fftw_plan plan_x_3D;
  // Process x field. x: RODFT10, y: REDFT10 z: REDFT10. 
  plan_x_3D = fftw_plan_r2r_3d(zRes_, yRes_, 2*xRes_, temp_, temp_,
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    (*coefficients)[i] += temp_[2*(K_[0] - 1) + K_[1]*2*xRes_ + K_[2]*2*SlabSize_]*invTotalSize_*PI_CUBE*0.125*C_x;
  }
  ClearTemp();
  // vy...
  // Put the value into field.
  ...
  fftw_plan plan_y_3D;
  // Process x field. x: REDFT10, y: RODFT10 z: REDFT10. 
  plan_y_3D = fftw_plan_r2r_3d(zRes_, yRes_, 2*xRes_, temp_, temp_,
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    int K0 = 2*K_[0] - 1;
    if (K0 < 0) K0 = 0;
    (*coefficients)[i] += temp_[K0 + (K_[1] - 1)*2*xRes_ + (K_[2])*2*SlabSize_]*invTotalSize_*PI_CUBE*0.125*C_y;
  }
  ClearTemp();
  // vz...
  // Put the value into field.
  ...
  for (uint k = 0; k < zRes_; k++) {
    for (uint j = 0; j < yRes_; j++) {
      for (uint i = 0; i < xRes_; i++) {
        uint tempid = i + j*2*xRes_ + k*2*SlabSize_;
        temp_[tempid] = field[index][2]; 
        index ++;
      }
    }
  }
  // Do a transformation inplace on vz.
  fftw_plan plan_z_3D;
  // Process x field. x: REDFT10, y: REDFT10 z: RODFT10. 
  plan_z_3D = fftw_plan_r2r_3d(zRes_, yRes_, 2*xRes_, temp_, temp_,
  ...
  for (int i = 0; i < basis_dim_; i++) {
    ...
    int K0 = 2*K_[0] - 1;
    if (K0 < 0) K0 = 0;
     (*coefficients)[i] += temp_[K0 + (K_[1])*2*xRes_ + (K_[2] - 1)*2*SlabSize_]*invTotalSize_*PI_CUBE*0.125*C_z;
  }
}

void OneNeumannBasisSet3D::InverseTramsformToVelocity(
  const Eigen::VectorXd& coefficients, VECTOR3_FIELD_3D* field) {
  if (! basis_allocated_) {std::cout << "one_neumann_basis_set_3d.cpp " << __LINE__ << " FATAL: "  << "Basis not allocated." << std::endl; exit(0);}
  field->clear();
  ClearTemp();
  ...
  for (int i = 0; i < coefficients.size(); i ++) {
    ...
    temp_[2*(K_[0] - 1) + K_[1]*2*xRes_ + K_[2]*2*SlabSize_] = coefficients[i] * C_x;
  }
  fftw_plan plan_x_3D;
  // x: RODFT01, y: REDFT01 z: REDFT01. 
  plan_x_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_*2, temp_, temp_, 
  ...
  for (int i = 0; i < coefficients.size(); i ++) {
    ...
    if (K0 < 0) K0 = 0;
    // Process y field.
    temp_[K0 + (K_[1] - 1)*2*xRes_ + (K_[2])*2*SlabSize_] = coefficients[i] * C_y;
  }
  fftw_plan plan_y_3D;
  // x: REDFT01, y: RODFT01 z: REDFT01. 
  plan_y_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_*2, temp_, temp_, 
  ...
  for (int i = 0; i < coefficients.size(); i ++) {
    ...
    int K0 = 2*K_[0] - 1;
    if (K0 < 0) K0 = 0;
    temp_[K0 + (K_[1])*2*xRes_ + (K_[2] - 1)*2*SlabSize_] = coefficients[i] * C_z;
  }
  fftw_plan plan_z_3D;
  // x: REDFT01, y: REDFT01 z: RODFT01. 
  plan_z_3D = fftw_plan_r2r_3d(zRes_, yRes_, xRes_*2, temp_, temp_, 
