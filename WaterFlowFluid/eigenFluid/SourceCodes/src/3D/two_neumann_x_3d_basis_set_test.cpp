...
void AllocateBasisTest(std::vector<TwoNeumannXBasis3D>* all_basis_) {
  // -k1*A + k2*B + k3*C = 0.
  //Test purpose
  TwoNeumannXBasis3D basis(1,4,0,4,1,0);
  all_basis_->push_back(basis);
  
  TwoNeumannXBasis3D basis1(4,0,1,1,0,4);
  all_basis_->push_back(basis1);
  
  TwoNeumannXBasis3D basis2(0,8,9,1,9,-8);
  all_basis_->push_back(basis2);
  
  TwoNeumannXBasis3D basis3(12,14,9,-5,-3,-2);
  all_basis_->push_back(basis3);
  
  TwoNeumannXBasis3D basis4(23,5,15,10,-8,18);
  all_basis_->push_back(basis4);
  
  TwoNeumannXBasis3D basis5(0,1,0,1,0,0);
  all_basis_->push_back(basis5);
  
  TwoNeumannXBasis3D basis6(0,0,1,1,0,0);
  all_basis_->push_back(basis6);
  
  TwoNeumannXBasis3D basis7(0,0,0,1,0,0);  // const.
...
void TransformationTest() {
  ...
  TwoNeumannXBasisSet3D basis_set(basis_dim, xRes, yRes, zRes, "principle_x");
  Timer timer;
...
void NormalizationTest() {
  ...
  TwoNeumannXBasisSet3D basis_set(basis_dim_des, xRes, yRes, zRes, "principle_x");
