...
void TwoNeumannXBasis3D::Normalize() {
  int num_zeros = 0;
  if (k1_ == 0) num_zeros ++;
  if (k2_ == 0) num_zeros ++;
  if (k3_ == 0) num_zeros ++;
  ...
  if (num_zeros == 3) {
    if (std::abs(A_) > 1e-15) {
      invNorm_ = 1.0 / (A_*sqrt(PI_CUBE));
    }
  }
}

double TwoNeumannXBasis3D::ComputeBasisAt(const int xpos, const int ypos,
...
void TwoNeumannXBasis3D::DiscretizeAdd(const double coef,
  ...
  for (int k = 0; k < zRes; k++) {
    for (int j = 0; j < yRes; j++) {
      for (int i = 0; i < xRes; i++) {
        vx = 0; vy = 0; vz = 0;
        if (A_ != 0) {
          vx = invNorm_*A_*cos((i+0.5)*k1_*dx_)*cos((j+0.5)*k2_*dy_)*cos((k+0.5)*k3_*dz_);
        }
        if (k2_ != 0 && k1_ != 0 && B_ != 0) {
          vy = invNorm_*B_*sin((i+0.5)*k1_*dx_)*sin((j+0.5)*k2_*dy_)*cos((k+0.5)*k3_*dz_);
        }
        if (k3_ != 0 && k1_ != 0 && C_ != 0) {
          vz = invNorm_*C_*sin((i+0.5)*k1_*dx_)*cos((j+0.5)*k2_*dy_)*sin((k+0.5)*k3_*dz_);
        }
