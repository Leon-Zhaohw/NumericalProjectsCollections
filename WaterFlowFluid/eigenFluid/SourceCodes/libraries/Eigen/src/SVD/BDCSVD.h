                                         ...
                                         VectorType& singVals, ArrayRef shifts, ArrayRef mus)
{
  ...
  for (Index k = 0; k < n; ++k)
  {
    ...
    if (useBisection)
    {
      ...
      if(!(fLeft * fRight<0))
      {
        std::cout <<  "fLeft: " << leftShifted << " - " << diagShifted.head(10).transpose()  << "\n  << std::endl; " << bool(left==shift) << " " << (left-shift) << "\n";
        std::cout <<  k << " : " <<  fLeft << " * " << fRight << " == " << fLeft * fRight << "   << std::endl;  " << left << " - " << right << " -> " <<  leftShifted << " " << rightShifted << "   shift=" << shift << "\n";
      }
#endif
...
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
      std::cout <<  "zhat(" << k << ") =  sqrt( " << prod << ")   << std::endl;  " << (singVals(last) + dk) << " * " << mus(last) + shifts(last) << " - " << dk << "\n";
#endif
...
void BDCSVD<MatrixType>::deflation44(Index firstColu , Index firstColm, Index firstRowW, Index firstColW, Index i, Index j, Index size)
{
  ...
  RealScalar r = sqrt(numext::abs2(c) + numext::abs2(s));
#ifdef  EIGEN_BDCSVD_DEBUG_VERBOSE
  std::cout <<  "deflation 4.4: " << i << "," << j << " -> " << c << " " << s << " " << r << "  << std::endl; "
