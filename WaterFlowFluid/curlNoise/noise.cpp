...
float Noise4::
operator()(float x, float y, float z, float t) const
{
   float floorx=std::floor(x), floory=std::floor(y), floorz=std::floor(z), floort=std::floor(t);
   int i=(int)floorx, j=(int)floory, k=(int)floorz, l=(int)floort;
   const Vec4f &n0000=basis[hash_index(i,j,k,l)];
   const Vec4f &n1000=basis[hash_index(i+1,j,k,l)];
   const Vec4f &n0100=basis[hash_index(i,j+1,k,l)];
   const Vec4f &n1100=basis[hash_index(i+1,j+1,k,l)];
   const Vec4f &n0010=basis[hash_index(i,j,k+1,l)];
   const Vec4f &n1010=basis[hash_index(i+1,j,k+1,l)];
   const Vec4f &n0110=basis[hash_index(i,j+1,k+1,l)];
   const Vec4f &n1110=basis[hash_index(i+1,j+1,k+1,l)];
   const Vec4f &n0001=basis[hash_index(i,j,k,l+1)];
   const Vec4f &n1001=basis[hash_index(i+1,j,k,l+1)];
   const Vec4f &n0101=basis[hash_index(i,j+1,k,l+1)];
   const Vec4f &n1101=basis[hash_index(i+1,j+1,k,l+1)];
   const Vec4f &n0011=basis[hash_index(i,j,k+1,l+1)];
   const Vec4f &n1011=basis[hash_index(i+1,j,k+1,l+1)];
   const Vec4f &n0111=basis[hash_index(i,j+1,k+1,l+1)];
   const Vec4f &n1111=basis[hash_index(i+1,j+1,k+1,l+1)];
   float fx=x-floorx, fy=y-floory, fz=z-floorz, ft=t-floort;
   float sx=fx*fx*fx*(10-fx*(15-fx*6)),
         sy=fy*fy*fy*(10-fy*(15-fy*6)),
         sz=fz*fz*fz*(10-fz*(15-fz*6)),
