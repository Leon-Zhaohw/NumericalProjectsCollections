...
//////////////////////////////////////////////////////////////////////////////////////////
static void downsampleX(Real *from, Real *to, int n){
  // if these values are not local incorrect results are generated
  Real downCoeffs[32] = { DOWNCOEFFS };
	const Real *a = &downCoeffs[16];
	for (int i = 0; i < n / 2; i++) {
		to[i] = 0;
...
static void downsampleY(Real *from, Real *to, int n){
  // if these values are not local incorrect results are generated
  Real downCoeffs[32] = { DOWNCOEFFS };
	const Real *a = &downCoeffs[16];
	for (int i = 0; i < n / 2; i++) {
...
static void downsampleZ(Real *from, Real *to, int n){
  // if these values are not local incorrect results are generated
  Real downCoeffs[32] = { DOWNCOEFFS };
	const Real *a = &downCoeffs[16];
	for (int i = 0; i < n / 2; i++) {
...
static void downsampleNeumann(const Real *from, Real *to, int n, int stride)
{
  // if these values are not local incorrect results are generated
  Real downCoeffs[32] = { DOWNCOEFFS };
  static const Real *const aCoCenter= &downCoeffs[16];
	for (int i = 0; i < n / 2; i++) {
		to[i * stride] = 0;
		for (int k = 2 * i - 16; k < 2 * i + 16; k++) { 
			// handle boundary
...
static void downsampleXNeumann(Real* to, const Real* from, int sx,int sy, int sz) {
...
static void downsampleYNeumann(Real* to, const Real* from, int sx,int sy, int sz) {
...
static void downsampleZNeumann(Real* to, const Real* from, int sx,int sy, int sz) {
	for (int ix = 0; ix < sx; ix++) 
		for (int iy = 0; iy < sy; iy++) {
...
//////////////////////////////////////////////////////////////////////////////////////////
static Real _upCoeffs[4] = {0.25f, 0.75f, 0.75f, 0.25f};
static void upsampleX(Real *from, Real *to, int n) {
	const Real *p = &_upCoeffs[2];
...
static void upsampleY(Real *from, Real *to, int n) {
	const Real *p = &_upCoeffs[2];
...
static void upsampleZ(Real *from, Real *to, int n) {
	const Real *p = &_upCoeffs[2];
...
static void upsampleNeumann(const Real *from, Real *to, int n, int stride) {
  static const Real *const pCoCenter = &_upCoeffs[2];
	for (int i = 0; i < n; i++) {
		to[i * stride] = 0;
		for (int k = i / 2; k <= i / 2 + 1; k++) {
			Real fromval;
...
static void upsampleXNeumann(Real* to, const Real* from, int sx, int sy, int sz) {
	for (int iy = 0; iy < sy; iy++) 
		for (int iz = 0; iz < sz; iz++) {
			const int i = iy * sx + iz*sx*sy;
...
static void upsampleYNeumann(Real* to, const Real* from, int sx, int sy, int sz) {
	for (int ix = 0; ix < sx; ix++) 
		for (int iz = 0; iz < sz; iz++) {
			const int i = ix + iz*sx*sy;
...
static void upsampleZNeumann(Real* to, const Real* from, int sx, int sy, int sz) {
	for (int ix = 0; ix < sx; ix++) 
		for (int iy = 0; iy < sy; iy++) {
			const int i = ix + iy*sx;
...
//////////////////////////////////////////////////////////////////////////////////////////
static bool loadTile(Real* const noiseTileData, std::string filename)
{
	FILE* file;
	...
	int bread = fread((void*)noiseTileData, sizeof(Real), gridSize, file);
	fclose(file);
...
//////////////////////////////////////////////////////////////////////////////////////////
static void saveTile(Real* const noiseTileData, std::string filename)
{
	FILE* file;
	...
	fwrite((void*)noiseTileData, sizeof(Real), noiseTileSize * noiseTileSize * noiseTileSize, file);
	fclose(file);
...
//////////////////////////////////////////////////////////////////////////////////////////
//static void generateTile(Real* const noiseTileData, std::string filename) {
static void generateTile(Real* const noiseTileData, std::string filename, int seed = 123456) {
...
static inline Real WNoise(VEC3F p, Real* data)
{ 
  int f[3], c[3], mid[3], n=noiseTileSize;
