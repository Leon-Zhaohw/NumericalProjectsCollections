...
#include "CUBATURE_GENERATOR_COMPRESSED_EIGEN.h"
#include <float.h>

//////////////////////////////////////////////////////////////////////
// Constructor for tetrahedra
//////////////////////////////////////////////////////////////////////
CUBATURE_GENERATOR_COMPRESSED_EIGEN::CUBATURE_GENERATOR_COMPRESSED_EIGEN(SUBSPACE_FLUID_3D_COMPRESSED_EIGEN* fluid,
    const char* filename) :
...
{
  cout << " Cubature generator received a fluid with peeled dims: " << fluid->xPeeled() << " " << fluid->yPeeled() << " " << fluid->zPeeled() << endl;
...
//////////////////////////////////////////////////////////////////////
void CUBATURE_GENERATOR_COMPRESSED_EIGEN::writeCubature(const string& filename)
{
...
//////////////////////////////////////////////////////////////////////
void CUBATURE_GENERATOR_COMPRESSED_EIGEN::pickImportanceSampledCandidatesOMP(const int pass, map<int, bool> alreadyUsed, const VectorXd& residual, vector<int>& candidates, int totalCandidates)
{
...
//////////////////////////////////////////////////////////////////////
VectorXd CUBATURE_GENERATOR_COMPRESSED_EIGEN::getStagedTrainingColumn(int index)
{
  TIMER functionTimer(__FUNCTION__);
  ...
  auto preU = _fluid->U_preadvect_data();
  const MatrixXd& postU = _fluid->prediffuseU();

  // submatrix of _Ubasis corresponding to this tet
  MatrixXd cellPreU = _fluid->cellBasisCompressedPeeled(preU, index);
  MatrixXd cellPostU = _fluid->cellBasisPeeled(postU, index);
  
  auto dataX = preU.get_compression_dataX();
  auto numCols = dataX->get_numCols();

  MatrixXd submatrix(3, numCols);

  // ADJ: old code that does not use caching
  ...
  for (unsigned int x = 0; x < _trainingPostadvection.size(); x++)
  {
    ...
    VectorXd postVelocity = _fluid->advectCellStamPeeled(preU, cellPreU, dt0, _trainingPreadvection[x], index, &submatrix);
...
//////////////////////////////////////////////////////////////////////////////
VectorXd CUBATURE_GENERATOR_COMPRESSED_EIGEN::matlabFNNLS(const MatrixXd& A, const VectorXd& b, bool verbose)
{
  TIMER functionTimer(__FUNCTION__);
...
// Bro and Jong fast modification
//////////////////////////////////////////////////////////////////////////////
VectorXd CUBATURE_GENERATOR_COMPRESSED_EIGEN::matlabFNNLS(const vector<VectorXd>& columns, const VectorXd& b, bool verbose)
{
  TIMER functionTimer(__FUNCTION__);
  cout << " Calling Matlab fast non-negative least squares ... " << flush;
  string matlabBin("/Applications/MATLAB_R2011a.app/bin/matlab");
...
//////////////////////////////////////////////////////////////////////
void CUBATURE_GENERATOR_COMPRESSED_EIGEN::cullCubatureZeros(VectorXd& weights, vector<int>& candidates)
{
...
//////////////////////////////////////////////////////////////////////
void CUBATURE_GENERATOR_COMPRESSED_EIGEN::generateImportanceSampledCubature()
{
  TIMER functionTimer(__FUNCTION__);
  ...
  {
    printf("On pass %i of a maximum of %i passes.\n", x+1, totalPasses);
