...
// Copied over from nnls.c so i don't have copyright on this
//...somebody else has.....don't know if this should be under GPL or LGPL or
// whatever, but i'll put a little note here anyways:
...
//////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
VectorXd NNLS_SOLVER::solve(const vector<VectorXd>& columns, const VectorXd& b)
{
  TIMER functionTimer("NNLS::solve");
  if (_workA) delete[] _workA;
  if (_workW) delete[] _workW;
  if (_workZ) delete[] _workZ;
  if (_workIndex) delete[] _workIndex;

  // allocate member vars now
  _rows = b.size();
  _maxCols = columns.size();
  ...
  // allocate A,x and b now too
  double* bNNLS = new double[_rows];
  double* xNNLS = new double[_maxCols];

  // copy the columns into the format the solver expects (Fortran ordering)
  int index = 0;
  for (unsigned int i = 0; i < columns.size(); i++)
    for (int j = 0; j < _rows; j++, index++)
      _workA[index] = columns[i][j];

  for (int i = 0; i < _rows; i++)
    bNNLS[i] = b[i];

  // call the Lawson-Hanson solver
  double rNorm = b.norm();

  // don't call this -- it makes unnecessary copies of A
  //bool success = solve(A, columns.size(), bNNLS, xNNLS, rNorm);

  // instead call NNLS directly
  int workMode;
	nnls(_workA, _rows, _rows, _maxCols, bNNLS, xNNLS, &rNorm,
			 _workW, _workZ, _workIndex, &workMode, _maxIter);
  bool success = workMode == 1;
  ...
  if (!success)
  {
    ...
    cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
    cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
    cout << " NNLS SOLVER BOMBED " << endl;
    cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
    cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
    cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
  }

  // copy results into some predictable form
  VectorXd xFinal(_maxCols);
...
//////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
VectorXd NNLS_SOLVER::solve(const MatrixXd& A, const VectorXd& b)
{
  TIMER functionTimer("NNLS::solve");
  if (_workA) delete[] _workA;
  if (_workW) delete[] _workW;
